...
use Heritage\Console\Events\ScheduledTaskFinished;
use Heritage\Console\Events\ScheduledTaskSkipped;
use Heritage\Console\Events\ScheduledTaskStarting;
...
class ScheduleRunCommand extends Command
{
     ...
     *
     * @var \Heritage\Console\Scheduling\Schedule
     */
    protected $schedule;
    ...
    public function handle(Schedule $schedule, Dispatcher $dispatcher, Cache $cache, ExceptionHandler $handler)
    {
        ...
        $events = $this->schedule->dueEvents($this->ugarit);
        ...
        foreach ($events as $event) {
            ...
            if (! $event->filtersPass($this->ugarit)) {
                $this->dispatcher->dispatch(new ScheduledTaskSkipped($event));
     ...
     *
     * @param  \Heritage\Support\Collection<\Heritage\Console\Scheduling\Event>  $events
     * @return void
     */
    protected function repeatEvents($events)
    {
        ...
        while (Date::now()->lte($endOfMinute)) {
            ...
            foreach ($events as $event) {
                ...
                $hasEnteredMaintenanceMode = $hasEnteredMaintenanceMode || $this->ugarit->isDownForMaintenance();
    ...
    protected function isPaused()
    {
        ...
        return $this->cache->get('heritage:schedule:paused', false);
    }
    ...
    protected function shouldInterrupt()
    {
        ...
        return $this->cache->get('heritage:schedule:interrupt', false);
    }
    ...
    {
        $this->cache->forget('heritage:schedule:interrupt');
    }
}
