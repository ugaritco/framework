...
class ScheduleListCommand extends Command
{
     ...
     *
     * @param  \Heritage\Support\Collection  $events
    ...
    private function listEvent($event, $terminalWidth, $expressionSpacing, $repeatExpressionSpacing, $timezone, $convertedExpression = null)
    {
        ...
        $command = preg_replace("#(php scribe [\w\-:]+) (.+)#", '$1 <fg=yellow;options=bold>$2</>', $command);
     ...
     */
    private function sortEvents(\Heritage\Support\Collection $events, DateTimeZone $timezone)
    {
     ...
     *
     * @param  \Heritage\Console\Scheduling\CallbackEvent  $event
     * @return string
     */
    private function getClosureLocation(CallbackEvent $event)
    {
        ...
        if ($callback instanceof Closure) {
            ...
            return sprintf(
                '%s:%s',
                str_replace($this->ugarit->basePath().DIRECTORY_SEPARATOR, '', $function->getFileName() ?: ''),
