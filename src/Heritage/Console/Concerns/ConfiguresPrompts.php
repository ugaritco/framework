...
namespace Heritage\Console\Concerns;

use Heritage\Console\PromptValidationException;
...
trait ConfiguresPrompts
{
    ...
    protected function configurePrompts(InputInterface $input)
    {
        ...
        Prompt::interactive(($input->isInteractive() && defined('STDIN') && stream_isatty(STDIN)) || $this->ugarit->runningUnitTests());

        Prompt::validateUsing(fn (Prompt $prompt) => $this->validatePrompt($prompt->value(), $prompt->validate));

        Prompt::fallbackWhen(windows_os() || $this->ugarit->runningUnitTests());
     ...
     * @return PResult
     *
     * @throws \Heritage\Console\PromptValidationException
     */
    protected function promptUntilValid($prompt, $required, $validate)
    {
        while (true) {
            ...
            if ($required && ($result === '' || $result === [] || $result === false)) {
                ...
                if ($this->ugarit->runningUnitTests()) {
    ...
    {
        return $this->ugarit['validator']->make(
            [$field => $value],
            [$field => $rules],
    ...
    private function multiselectFallback($label, $options, $default = [], $required = false)
    {
        ...
        if ($required === false && ! $this->ugarit->runningUnitTests()) {
