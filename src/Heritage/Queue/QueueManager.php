...
use Closure;
use Heritage\Contracts\Queue\Factory as FactoryContract;
use Heritage\Contracts\Queue\Monitor as MonitorContract;
...
/**
 * @mixin \Heritage\Contracts\Queue\Queue
 */
class QueueManager implements FactoryContract, MonitorContract
{
     ...
     *
     * @param  string  $driver
     * @return \Heritage\Queue\Connectors\ConnectorInterface
     *
     * @throws \InvalidArgumentException
     */
    ...
    public function pause($connection, $queue)
    {
        ...
        $this->app['cache']
            ->store()
            ->forever("heritage:queue:paused:{$connection}:{$queue}", true);
    ...
    public function pauseFor($connection, $queue, $ttl)
    {
        ...
        $this->app['cache']
            ->store()
            ->put("heritage:queue:paused:{$connection}:{$queue}", true, $ttl);
    ...
    public function pauseAll()
    {
        $this->app['cache']
            ->store()
            ->forever('heritage:queues:paused', true);
    ...
    public function resume($connection, $queue)
    {
        ...
        $this->app['cache']
            ->store()
            ->forget("heritage:queue:paused:{$connection}:{$queue}");
    ...
    public function isPaused($connection, $queue)
    {
        ...
        return (bool) ($cache->get('heritage:queues:paused')
            ?: $cache->get("heritage:queue:paused:{$connection}:{$queue}"));
    }
    ...
    public function getPausedQueues($connection, $queues)
    {
        ...
        if ($cache->get('heritage:queues:paused')) {
            return array_values($queues);
        }

        $states = $cache->many(
            array_map(fn ($queue) => "heritage:queue:paused:{$connection}:{$queue}", $queues)
        );

        return array_values(array_filter(
            $queues, fn ($queue) => $states["heritage:queue:paused:{$connection}:{$queue}"] ?? false
        ));
    }
