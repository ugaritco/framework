...
use Heritage\Queue\Events\JobRetryRequested;
...
class RetryCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($ids as $id) {
            $job = $this->ugarit['queue.failer']->find($id);

            if (is_null($job)) {
                $this->components->error("Unable to find failed job with ID [{$id}].");
            } else {
                $this->ugarit['events']->dispatch(new JobRetryRequested($job));

                $this->components->task($id, fn () => $this->retryJob($job));

                $this->ugarit['queue.failer']->forget($id);
            }
        }
    ...
    protected function getJobIds()
    {
        ...
        if (count($ids) === 1 && $ids[0] === 'all') {
            $failer = $this->ugarit['queue.failer'];
    ...
    {
        $queue = $this->ugarit['queue']->connection($job->connection);

        $this->ugarit['queue']->connection($job->connection)->pushRaw(
    ...
    protected function getInstanceFromPayload($payload)
    {
        ...
        if ($this->ugarit->bound(Encrypter::class)) {
            return unserialize($this->ugarit->make(Encrypter::class)->decrypt($payload['data']['command']));
        }
