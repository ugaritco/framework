...
class WorkCommand extends Command
{
     ...
     *
     * @param  \Heritage\Queue\Worker  $worker
     ...
     *
     * @return \Heritage\Queue\WorkerOptions
     */
    ...
    protected function listenForEvents()
    {
        ...
        $this->ugarit['events']->listen(JobProcessing::class, function ($event) {
            $this->writeOutput($event->job, 'starting');
        });

        $this->ugarit['events']->listen(JobProcessed::class, function ($event) {
            $this->writeOutput($event->job, 'success');
        });

        $this->ugarit['events']->listen(JobReleasedAfterException::class, function ($event) {
            $this->writeOutput($event->job, 'released_after_exception');
        });

        $this->ugarit['events']->listen(JobFailed::class, function ($event) {
            $this->writeOutput($event->job, 'failed', $event->exception);

            $this->logFailedJob($event);
        });

        $this->ugarit['events']->listen(WorkerQueuePaused::class, function ($event) {
            $this->writeQueueStatus($event->queue, 'paused');
        });

        $this->ugarit['events']->listen(WorkerQueueResumed::class, function ($event) {
            $this->writeQueueStatus($event->queue, 'resumed');
        });

        $this->ugarit['events']->listen(WorkerStopping::class, function ($event) {
     ...
     *
     * @param  \Heritage\Queue\Events\WorkerStopping  $event
     * @return void
     */
    ...
    {
        $queueTimezone = $this->ugarit['config']->get('queue.output_timezone');

        if ($queueTimezone &&
            $queueTimezone !== $this->ugarit['config']->get('app.timezone')) {
     ...
     *
     * @param  \Heritage\Queue\Events\JobFailed  $event
     * @return void
     */
    protected function logFailedJob(JobFailed $event)
    {
        $this->ugarit['queue.failer']->log(
    ...
    {
        return $this->option('force') ? false : $this->ugarit->isDownForMaintenance();
    }
