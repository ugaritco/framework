...
namespace Heritage\View\Compilers;
...
use Heritage\View\AnonymousComponent;
use Heritage\View\DynamicComponent;
use Heritage\View\ViewFinderInterface;
use InvalidArgumentException;
use ReflectionClass;

/**
 * @author Spatie bvba <info@spatie.be>
 * @author Taylor Otwell <taylor@ugarit.com>
 */
class ComponentTagCompiler
{
     ...
     * @param  \Heritage\View\Compilers\BladeCompiler|null  $blade
     */
    ...
    protected function componentString(string $component, array $attributes)
    {
        ...
        if (! class_exists($class)) {
            $view = Str::startsWith($component, 'mail::')
                ? "\$__env->getContainer()->make(Heritage\\View\\Factory::class)->make('{$component}')"
                : "'$class'";
        ...
        return preg_replace_callback(
            '/@(class)(\( ( (?>[^()]+) | (?2) )* \))/x', function ($match) {
                if ($match[1] === 'class') {
                    ...
                    return ":class=\"\Heritage\Support\Arr::toCssClasses{$match[2]}\"";
                }
        ...
        return preg_replace_callback(
            '/@(style)(\( ( (?>[^()]+) | (?2) )* \))/x', function ($match) {
                if ($match[1] === 'style') {
                    ...
                    return ":style=\"\Heritage\Support\Arr::toCssStyles{$match[2]}\"";
                }
        ...
        return (new Collection($attributes))
            ->map(function (string $value, string $attribute) use ($escapeBound) {
                return $escapeBound && isset($this->boundAttributes[$attribute]) && $value !== 'true' && ! is_numeric($value)
                    ? "'{$attribute}' => \Heritage\View\Compilers\BladeCompiler::sanitizeComponentAttribute({$value})"
                    : "'{$attribute}' => {$value}";
            })
            ->implode(',');
    }
