...
class BladeCompiler extends Compiler implements CompilerInterface
{
     ...
     *
     * @param  \Heritage\View\Component  $component
     * @return string
     */
    ...
    public function if($name, callable $callback)
    {
        ...
        $this->directive($name, function ($expression) use ($name) {
            return $expression !== ''
                ? "<?php if (\Heritage\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                : "<?php if (\Heritage\Support\Facades\Blade::check('{$name}')): ?>";
        });
        ...
        $this->directive('else'.$name, function ($expression) use ($name) {
            return $expression !== ''
                ? "<?php elseif (\Heritage\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                : "<?php elseif (\Heritage\Support\Facades\Blade::check('{$name}')): ?>";
        });
