...
class DynamicComponent extends Component
{
     ...
     *
     * @var \Heritage\View\Compilers\ComponentTagCompiler
     */
     ...
     *
     * @return \Heritage\Contracts\View\View|string
     */
    public function render()
    {
        $template = <<<'EOF'
<?php extract((new \Heritage\Support\Collection($attributes->getAttributes()))->mapWithKeys(function ($value, $key) { return [Heritage\Support\Str::camel(str_replace([':', '.'], ' ', $key)) => $value]; })->all(), EXTR_SKIP); ?>
{{ props }}
...
EOF;
        ...
        return function ($data) use ($template) {
            ...
            return str_replace(
                ...
                [
                    ...
                    $this->compileSlots($data['__ugarit_slots']),
                    '{{ $slot ?? "" }}',
                ],
                $template
            );
     ...
     *
     * @return \Heritage\View\Compilers\ComponentTagCompiler
     */
