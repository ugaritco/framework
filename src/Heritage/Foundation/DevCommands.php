...
use Heritage\Support\NodePackageManager;
use Ugarit\Pail\PailServiceProvider;
use ReflectionClass;
...
class DevCommands
{
    ...
    public static function registerDefaults()
    {
        ...
        self::scribe('serve', 'server');
        self::scribe('queue:listen --tries=1 --timeout=0', 'queue');

        if (function_exists('pcntl_fork') && app()->providerIsLoaded(PailServiceProvider::class)) {
            self::scribe('pail --timeout=0', 'logs');
        }
    ...
    /**
     * Registers an Scribe command, automatically prefixing it with "php scribe".
     *
     * @param  string  $command
     * @param  string|null  $name
     * @return DevCommand
     */
    public static function scribe(string $command, ?string $name = null): DevCommand
    {
        return self::register("php scribe {$command}", $name ?? DevCommand::nameFromCommand($command));
    }
