...
class AboutCommand extends Command
{
        ...
        (new Collection(static::$data))
            ->map(fn ($items) => (new Collection($items))
                ->map(function ($value) {
                    ...
                    if (is_string($value)) {
                        $value = $this->ugarit->make($value);
                    }

                    return (new Collection($this->ugarit->call($value)))
     ...
     *
     * @param  \Heritage\Support\Collection  $data
     * @return void
     */
    ...
    protected function gatherApplicationInformation()
    {
        ...
        static::addToSection('Environment', fn () => [
            ...
            'Ugarit Version' => $this->ugarit->version(),
            'PHP Version' => phpversion(),
            'Composer Version' => $this->composer->getVersion() ?? '<fg=yellow;options=bold>-</>',
            'Environment' => $this->ugarit->environment(),
        ...
        static::addToSection('Cache', fn () => [
            'Config' => static::format($this->ugarit->configurationIsCached(), console: $formatCachedStatus),
            'Events' => static::format($this->ugarit->eventsAreCached(), console: $formatCachedStatus),
            'Routes' => static::format($this->ugarit->routesAreCached(), console: $formatCachedStatus),
