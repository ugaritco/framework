...
use Heritage\Http\Client\Factory as Http;
...
class DocsCommand extends Command
{
     ...
     * @var string
     */
    protected $description = 'Access the Ugarit documentation';
     ...
     * @var string
     */
    protected $help = 'If you would like to perform a content search against the documentation, you may call: <fg=green>php scribe docs -- </><fg=green;options=bold;>search query here</>';
     ...
     *
     * @param  \Heritage\Http\Client\Factory  $http
    ...
    {
        if ($this->isSearching()) {
            return "https://ugarit.com/docs/{$this->version()}?".Arr::query([
                'q' => $this->searchQuery(),
            ]);
        }
        ...
        return trim("https://ugarit.com/docs/{$this->version()}/{$page}#{$this->section($page)}", '#/');
    }
    ...
    {
        try {
            $strategy = require Env::get('SCRIBE_DOCS_ASK_STRATEGY');
        } catch (Throwable) {
            return null;
        }
    ...
    protected function open($url)
    {
        ($this->urlOpener ?? function ($url) {
            if (Env::get('SCRIBE_DOCS_OPEN_STRATEGY')) {
    ...
    {
        try {
            $command = require Env::get('SCRIBE_DOCS_OPEN_STRATEGY');
        } catch (Throwable) {
            $command = null;
        }
    ...
    public function docs()
    {
        return $this->cache->remember(
            "scribe.docs.{{$this->version()}}.index",
    ...
    protected function refreshDocs()
    {
        ...
        if ($response->successful()) {
            $this->cache->put("scribe.docs.{{$this->version()}}.index", $response->collect(), CarbonInterval::months(2));
        }
    }

    /**
     * Fetch the documentation index from the Ugarit website.
     *
     * @return \Heritage\Http\Client\Response
     */
    protected function fetchDocs()
    {
        return $this->http->get("https://ugarit.com/docs/{$this->version()}/index.json");
    }
    ...
    {
        return Str::before($this->version ?? $this->ugarit->version(), '.').'.x';
    }
