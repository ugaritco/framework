...
class EnvironmentDecryptCommand extends Command
{
    ...
    {
        $key = $this->option('key') ?: Env::get('UGARIT_ENV_ENCRYPTION_KEY');
        ...
        $encryptedFile = ($this->option('env')
            ? Str::finish($this->ugarit->environmentPath(), DIRECTORY_SEPARATOR).'.env.'.$this->option('env')
            : $this->ugarit->environmentFilePath()).'.encrypted';
     ...
     *
     * @param  string  $contents
     * @param  \Heritage\Encryption\Encrypter  $encrypter
     * @return string
     */
    ...
    {
        $path = Str::finish($this->option('path') ?: $this->ugarit->environmentPath(), DIRECTORY_SEPARATOR);
