...
/**
 * @phpstan-import-type DevCommandArray from \Heritage\Foundation\DevCommands
 */
#[AsCommand(name: 'dev')]
class DevCommand extends Command
{
    ...
    /**
     * Run the dev commands via `@ugarit/multiplex`.
     *
    ...
    /**
     * Build the command to run `@ugarit/multiplex` with the given dev commands.
     *
     * @param  DevCommandArray[]  $devCommands
     * @return string
     */
    protected function buildMultiplexCommand(array $devCommands): string
    {
        ...
        $title = 'scribe dev · '.(config('app.name') === 'Ugarit' ? basename(base_path()) : config('app.name'));

        $command = '@ugarit/multiplex --title '.escapeshellarg($title);
