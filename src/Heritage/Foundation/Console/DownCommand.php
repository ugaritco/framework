...
use Heritage\Foundation\Exceptions\RegisterErrorViewPaths;
...
class DownCommand extends Command
{
    ...
    {
        try {
            $wasAlreadyDown = $this->ugarit->maintenanceMode()->active();

            $downFilePayload = $this->getDownFilePayload();

            $this->ugarit->maintenanceMode()->activate($downFilePayload);
            ...
            $this->ugarit->get('events')->dispatch(new MaintenanceModeEnabled());
    ...
    {
        try {
            return $this->ugarit->make(AppPreventRequestsDuringMaintenance::class)->getExcludedPaths();
        } catch (Throwable) {
            try {
                return $this->ugarit->make(PreventRequestsDuringMaintenance::class)->getExcludedPaths();
            } catch (Throwable) {
                return [];
            }
        }
