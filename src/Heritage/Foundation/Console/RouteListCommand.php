...
use Heritage\Routing\ViewController;
...
class RouteListCommand extends Command
{
    ...
    protected function isFrameworkController(Route $route)
    {
        return in_array($route->getControllerClass(), [
            '\Heritage\Routing\RedirectController',
            '\Heritage\Routing\ViewController',
        ], true);
    }
     ...
     *
     * @param  \Heritage\Support\Collection  $routes
     * @return string
     */
    ...
    protected function formatActionForCli($route)
    {
        ...
        $rootControllerNamespace = $this->ugarit[UrlGenerator::class]->getRootControllerNamespace()
            ?? ($this->ugarit->getNamespace().'Http\\Controllers');
