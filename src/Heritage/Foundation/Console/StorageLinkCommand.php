...
class StorageLinkCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($this->links() as $link => $target) {
            ...
            if (is_link($link)) {
                $this->ugarit->make('files')->delete($link);
            }

            if ($relative) {
                $this->ugarit->make('files')->relativeLink($target, $link);
            } else {
                $this->ugarit->make('files')->link($target, $link);
            }
    ...
    {
        return $this->ugarit['config']['filesystems.links'] ??
