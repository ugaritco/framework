...
class ApiInstallCommand extends Command
{
    ...
    protected $signature = 'install:api
                    ...
                    {--passport : Install Ugarit Passport instead of Ugarit Sanctum}
     ...
     * @var string
     */
    protected $description = 'Create an API routes file and install Ugarit Sanctum or Ugarit Passport';
    ...
    public function handle()
    {
        ...
        if (file_exists($apiRoutesPath = $this->ugarit->basePath('routes/api.php')) &&
            ! $this->option('force')) {
        ...
        if ($this->option('passport')) {
            ...
            $this->components->info('API scaffolding installed. Please add the [Ugarit\Passport\HasApiTokens] trait to your User model.');
        } else {
            ...
            $this->components->info('API scaffolding installed. Please add the [Ugarit\Sanctum\HasApiTokens] trait to your User model.');
        }
    }
    ...
    /**
     * Install Ugarit Sanctum into the application.
     *
     * @return void
     */
    protected function installSanctum()
    {
        ...
        $migrationPublished = (new Collection(scandir($this->ugarit->databasePath('migrations'))))->contains(function ($migration) {
        ...
        if (! $migrationPublished) {
            Process::run([
                ...
                'vendor:publish',
                '--provider',
                'Ugarit\\Sanctum\\SanctumServiceProvider',
            ]);
        }
    }

    /**
     * Install Ugarit Passport into the application.
     *
     * @return void
     */
