...
<div class="flex flex-col pt-8 sm:pt-16 overflow-x-auto">
    <div class="flex flex-col gap-5 mb-8">
        ...
        <x-ugarit-exceptions-renderer::file-with-line :frame="$exception->frames()->first()" class="-mt-3 text-xs" />
    ...
    <div class="flex items-start gap-2 mb-8 sm:mb-16">
        <div class="bg-white dark:bg-white/[3%] border border-neutral-200 dark:border-white/10 divide-x divide-neutral-200 dark:divide-white/10 rounded-md shadow-xs flex items-center gap-0.5">
            <div class="flex items-center gap-1.5 h-6 px-[6px] font-mono text-[13px]">
                <span class="text-neutral-400 dark:text-neutral-500">UGARIT</span>
        ...
        <x-ugarit-exceptions-renderer::badge type="error">
            <x-ugarit-exceptions-renderer::icons.alert class="w-2.5 h-2.5" />
            UNHANDLED
        </x-ugarit-exceptions-renderer::badge>
        <x-ugarit-exceptions-renderer::badge type="error" variant="solid">
            CODE {{ $exception->code() }}
        </x-ugarit-exceptions-renderer::badge>
    </div>

    <x-ugarit-exceptions-renderer::request-url :$exception :request="$exception->request()" class="relative z-50" />
</div>
