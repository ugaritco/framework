...
>
    <div class="flex items-center justify-between p-2">
        <div class="flex items-center gap-2.5">
            <div class="bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-white/5 rounded-md w-6 h-6 flex items-center justify-center p-1">
                <x-ugarit-exceptions-renderer::icons.database class="w-2.5 h-2.5 text-blue-500 dark:text-emerald-500" />
            </div>
        ...
        <div x-show="totalQueries > 0" class="text-sm text-neutral-500 dark:text-neutral-400 flex items-center gap-2">
            ...
            @if (count($queries) > 100)
                <x-ugarit-exceptions-renderer::icons.info
    ...
    <div class="flex flex-col gap-1">
        ...
        >
            <div class="flex items-center gap-2 truncate">
                <div class="flex items-center gap-2">
                    <x-ugarit-exceptions-renderer::icons.database class="w-3 h-3 text-neutral-500 dark:text-neutral-400" />
                    <span class="text-neutral-500 dark:text-neutral-400">{{ $connectionName }}</span>
                </div>
                <x-ugarit-exceptions-renderer::syntax-highlight
                    :code="$sql"
                    language="sql"
                    truncate
                    class="min-w-0"
        ...
        @empty
        <x-ugarit-exceptions-renderer::empty-state message="No queries executed" />
        @endforelse
    </div>
    ...
    <div x-cloak x-show="totalPages > 1" class="flex items-center justify-center gap-1 py-4 font-mono">
        ...
        >
            <x-ugarit-exceptions-renderer::icons.chevrons-left class="w-3 h-3" />
        </button>
        ...
        <button
            ...
            :disabled="!hasPrevious"
        >
            <x-ugarit-exceptions-renderer::icons.chevron-left class="w-3 h-3" />
        </button>
        ...
        <button
            ...
            :disabled="!hasNext"
        >
            <x-ugarit-exceptions-renderer::icons.chevron-right class="w-3 h-3" />
        </button>
        ...
        <button
            ...
            :disabled="!hasNext"
        >
            <x-ugarit-exceptions-renderer::icons.chevrons-right class="w-3 h-3" />
        </button>
    </div>
</div>
