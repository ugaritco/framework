...
>
    ...
    >
        <x-ugarit-exceptions-renderer::icons.folder class="w-3 h-3 text-neutral-400" x-show="!expanded" x-cloak />
        <x-ugarit-exceptions-renderer::icons.folder-open class="w-3 h-3 text-blue-500 dark:text-emerald-500" x-show="expanded" />
        ...
        >
            ...
            <x-ugarit-exceptions-renderer::icons.chevrons-up-down x-show="!expanded" />
        </button>
    </div>
    ...
    <div x-cloak class="flex flex-col rounded-b-lg divide-y divide-neutral-200 border-t border-neutral-200 dark:divide-white/5 dark:border-white/5" x-show="expanded">
        @foreach ($frames as $frame)
            <div class="flex flex-col divide-y divide-neutral-200 dark:divide-white/5">
                <x-ugarit-exceptions-renderer::vendor-frame :$frame />
            </div>
        @endforeach
    </div>
</div>
