...
>
    ...
    >
        ...
        <div class="flex flex-1 items-center justify-between gap-6 min-w-0">
            <x-ugarit-exceptions-renderer::formatted-source :$frame />
            <x-ugarit-exceptions-renderer::file-with-line :$frame direction="rtl" />
        </div>
    ...
    @if($snippet = $frame->snippet())
        <x-ugarit-exceptions-renderer::frame-code :code="$snippet" :highlightedLine="$frame->line()" x-show="expanded" :x-cloak="!$frame->isMain()" />
    @endif
</div>
