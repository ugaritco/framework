...
use Heritage\Events\EventServiceProvider;
...
use Heritage\Log\Context\ContextServiceProvider;
use Heritage\Log\LogServiceProvider;
use Heritage\Routing\RoutingServiceProvider;
...
class Application extends Container implements ApplicationContract, CachesConfiguration, CachesRoutes, HttpKernelInterface
{
    ...
    /**
     * The base path for the Ugarit installation.
     *
     * @var string
     */
    protected $basePath;
     ...
     *
     * @var array<string, \Heritage\Support\ServiceProvider>
     */
    ...
    /**
     * Create a new Heritage application instance.
     *
    ...
    public function __construct($basePath = null)
    {
        ...
        $this->registerUgaritCloudServices();
    }

    /**
     * Begin configuring a new Ugarit application instance.
     *
     * @param  string|null  $basePath
     * @return \Heritage\Foundation\Configuration\ApplicationBuilder
     */
    ...
    /**
     * Register any services needed for Ugarit Cloud.
     *
     * @return void
     */
    protected function registerUgaritCloudServices()
    {
        if (! ugarit_cloud()) {
            return;
        }
    ...
    protected function bindPathsInContainer()
    {
        ...
        $this->useBootstrapPath(value(function () {
            return is_dir($directory = $this->basePath('.ugarit'))
                ? $directory
    ...
    {
        if (isset($_ENV['UGARIT_STORAGE_PATH'])) {
            return $this->joinPaths($this->storagePath ?: $_ENV['UGARIT_STORAGE_PATH'], $path);
        }

        if (isset($_SERVER['UGARIT_STORAGE_PATH'])) {
            return $this->joinPaths($this->storagePath ?: $_SERVER['UGARIT_STORAGE_PATH'], $path);
        }
    ...
    public function registerConfiguredProviders()
    {
        $providers = (new Collection($this->make('config')->get('app.providers')))
            ->partition(fn ($provider) => str_starts_with($provider, 'Heritage\\'));
     ...
     *
     * @param  \Heritage\Support\ServiceProvider|string  $provider
     * @return \Heritage\Support\ServiceProvider|null
     */
     ...
     *
     * @param  \Heritage\Support\ServiceProvider  $provider
     * @return void
     */
     ...
     *
     * @throws \Heritage\Contracts\Container\BindingResolutionException
     * @throws \Heritage\Contracts\Container\CircularDependencyException
     */
    ...
    /**
     * Handle the incoming Scribe command.
     *
    ...
    {
        foreach ([
            'app' => [self::class, \Heritage\Contracts\Container\Container::class, \Heritage\Contracts\Foundation\Application::class, \Psr\Container\ContainerInterface::class],
            'auth' => [\Heritage\Auth\AuthManager::class, \Heritage\Contracts\Auth\Factory::class],
            'auth.driver' => [\Heritage\Contracts\Auth\Guard::class],
            'auth.password' => [\Heritage\Auth\Passwords\PasswordBrokerManager::class, \Heritage\Contracts\Auth\PasswordBrokerFactory::class],
            'auth.password.broker' => [\Heritage\Auth\Passwords\PasswordBroker::class, \Heritage\Contracts\Auth\PasswordBroker::class],
            'blade.compiler' => [\Heritage\View\Compilers\BladeCompiler::class],
            'cache' => [\Heritage\Cache\CacheManager::class, \Heritage\Contracts\Cache\Factory::class],
            'cache.store' => [\Heritage\Cache\Repository::class, \Heritage\Contracts\Cache\Repository::class, \Psr\SimpleCache\CacheInterface::class],
            'cache.psr6' => [\Symfony\Component\Cache\Adapter\Psr16Adapter::class, \Symfony\Component\Cache\Adapter\AdapterInterface::class, \Psr\Cache\CacheItemPoolInterface::class],
            'config' => [\Heritage\Config\Repository::class, \Heritage\Contracts\Config\Repository::class],
            'cookie' => [\Heritage\Cookie\CookieJar::class, \Heritage\Contracts\Cookie\Factory::class, \Heritage\Contracts\Cookie\QueueingFactory::class],
            'db' => [\Heritage\Database\DatabaseManager::class, \Heritage\Database\ConnectionResolverInterface::class],
            'db.connection' => [\Heritage\Database\Connection::class, \Heritage\Database\ConnectionInterface::class],
            'db.schema' => [\Heritage\Database\Schema\Builder::class],
            'encrypter' => [\Heritage\Encryption\Encrypter::class, \Heritage\Contracts\Encryption\Encrypter::class, \Heritage\Contracts\Encryption\StringEncrypter::class],
            'events' => [\Heritage\Events\Dispatcher::class, \Heritage\Contracts\Events\Dispatcher::class],
            'files' => [\Heritage\Filesystem\Filesystem::class],
            'filesystem' => [\Heritage\Filesystem\FilesystemManager::class, \Heritage\Contracts\Filesystem\Factory::class],
            'filesystem.disk' => [\Heritage\Contracts\Filesystem\Filesystem::class],
            'filesystem.cloud' => [\Heritage\Contracts\Filesystem\Cloud::class],
            'hash' => [\Heritage\Hashing\HashManager::class],
            'hash.driver' => [\Heritage\Contracts\Hashing\Hasher::class],
            'image' => [\Heritage\Image\ImageManager::class],
            'log' => [\Heritage\Log\LogManager::class, \Psr\Log\LoggerInterface::class],
            'mail.manager' => [\Heritage\Mail\MailManager::class, \Heritage\Contracts\Mail\Factory::class],
            'mailer' => [\Heritage\Mail\Mailer::class, \Heritage\Contracts\Mail\Mailer::class, \Heritage\Contracts\Mail\MailQueue::class],
            'queue' => [\Heritage\Queue\QueueManager::class, \Heritage\Contracts\Queue\Factory::class, \Heritage\Contracts\Queue\Monitor::class],
            'queue.connection' => [\Heritage\Contracts\Queue\Queue::class],
            'queue.failer' => [\Heritage\Queue\Failed\FailedJobProviderInterface::class],
            'redirect' => [\Heritage\Routing\Redirector::class],
            'redis' => [\Heritage\Redis\RedisManager::class, \Heritage\Contracts\Redis\Factory::class],
            'redis.connection' => [\Heritage\Redis\Connections\Connection::class, \Heritage\Contracts\Redis\Connection::class],
            'request' => [\Heritage\Http\Request::class, \Symfony\Component\HttpFoundation\Request::class],
            'router' => [\Heritage\Routing\Router::class, \Heritage\Contracts\Routing\Registrar::class, \Heritage\Contracts\Routing\BindingRegistrar::class],
            'session' => [\Heritage\Session\SessionManager::class],
            'session.store' => [\Heritage\Session\Store::class, \Heritage\Contracts\Session\Session::class],
            'translator' => [\Heritage\Translation\Translator::class, \Heritage\Contracts\Translation\Translator::class],
            'url' => [\Heritage\Routing\UrlGenerator::class, \Heritage\Contracts\Routing\UrlGenerator::class],
            'validator' => [\Heritage\Validation\Factory::class, \Heritage\Contracts\Validation\Factory::class],
            'view' => [\Heritage\View\Factory::class, \Heritage\Contracts\View\Factory::class],
        ] as $key => $aliases) {
