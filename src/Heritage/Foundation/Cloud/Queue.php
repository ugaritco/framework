...
use Heritage\Contracts\Queue\Queue as QueueContract;
...
class Queue implements QueueContract, ClearableQueue
{
     ...
     *
     * @return \Heritage\Foundation\Cloud\CloudJob|null
     */
     ...
     *
     * @throws \Heritage\Foundation\Cloud\AgentUnreachableException
     */
    protected function requestNextJobFromAgent(): ?array
    {
        try {
            ...
            throw new AgentUnreachableException(
                'The Ugarit Cloud agent runtime socket is unreachable.', previous: $e
            );
        }
        ...
        if (! $response->ok()) {
            throw new AgentUnreachableException(
                "The Ugarit Cloud agent returned HTTP {$response->status()} from GET /next."
            );
        }

        if (! is_array($data = $response->json())) {
            throw new AgentUnreachableException(
                'The Ugarit Cloud agent returned a non-array body from GET /next.'
            );
        }
    ...
    protected function reportJobStatusToAgent(string $messageId, ?string $receiptHandle, string $status, ?int $delay = null): void
    {
        try {
            ...
            if ($e->response->serverError()) {
                throw new AgentUnreachableException(
                    "The Ugarit Cloud agent returned HTTP {$e->response->status()} from POST /result.", previous: $e
                );
            }
