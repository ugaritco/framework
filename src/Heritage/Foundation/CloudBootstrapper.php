...
use Heritage\Foundation\Bootstrap\BootProviders;
...
class CloudBootstrapper
{
    ...
    /**
     * Configure the Ugarit Cloud disks if applicable.
     */
    public static function configureDisks(Application $app): void
    {
        if (! isset($_SERVER['UGARIT_CLOUD_DISK_CONFIG'])) {
            return;
        }

        $defaultDisk = $_SERVER['FILESYSTEM_DISK'] ?? null;

        $disks = json_decode($_SERVER['UGARIT_CLOUD_DISK_CONFIG'], true);
    ...
    /**
     * Configure the unpooled Ugarit Postgres connection if applicable.
     */
    public static function configureUnpooledPostgresConnection(Application $app): void
    {
        ...
        if (str_contains($host, 'pg.ugarit.cloud') &&
    ...
    {
        if (! isset($_SERVER['UGARIT_CLOUD_MANAGED_QUEUES_CONFIG'])) {
            return;
        }

        $config = json_decode($_SERVER['UGARIT_CLOUD_MANAGED_QUEUES_CONFIG'], associative: true, flags: JSON_THROW_ON_ERROR);
    ...
    /**
     * Configure the Ugarit Cloud log channels.
     */
    public static function configureCloudLogging(Application $app): void
    {
        ...
        $channel = [
            ...
            'formatter' => UgaritCloudJsonFormatter::class,
            'formatter_with' => [
                'includeStacktraces' => true,
            ],
            'with' => [
        ...
        $app['config']->set('logging.channels.ugarit-cloud-socket', $channel);
    ...
    /**
     * Register the events system for Ugarit Cloud.
     */
    ...
    {
        return $_ENV['UGARIT_CLOUD_LOG_SOCKET'] ??
            $_SERVER['UGARIT_CLOUD_LOG_SOCKET'] ??
