...
use Heritage\Contracts\View\Factory as ViewFactoryContract;
    ...
    protected function registerPsrRequest()
    {
        $this->app->bind(ServerRequestInterface::class, function ($app) {
            if (class_exists(PsrHttpFactory::class)) {
                $heritageRequest = $app->make('request');
                $request = (new PsrHttpFactory)->createRequest($heritageRequest);

                if ($heritageRequest->getContentTypeFormat() !== 'json' && $heritageRequest->request->count() === 0) {
                    return $request;
                }

                return $request->withParsedBody(
                    array_merge($request->getParsedBody() ?? [], $heritageRequest->getPayload()->all())
                );
            }
