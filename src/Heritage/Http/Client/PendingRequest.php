...
namespace Heritage\Http\Client;
...
use Heritage\Http\Client\Events\ConnectionFailed;
use Heritage\Http\Client\Events\RequestSending;
use Heritage\Http\Client\Events\ResponseReceived;
use Heritage\Http\Client\Promises\FluentPromise;
use Heritage\Http\Client\Promises\LazyPromise;
...
class PendingRequest
{
     ...
     *
     * @var \Heritage\Http\Client\Factory|null
     */
    protected $factory;
    ...
    /**
     * The callbacks that should execute after the Ugarit Response is built.
     *
     * @var \Heritage\Support\Collection<int, (callable(\Heritage\Http\Client\Response, \Heritage\Http\Client\Request): \Heritage\Http\Client\Response|null)>
     */
     ...
     *
     * @var \Heritage\Support\Collection|null
     */
     ...
     *
     * @var \Heritage\Http\Client\Request|null
     */
    protected $request;
     ...
     *
     * @param  \Heritage\Http\Client\Factory|null  $factory
     ...
     *
     * @param  callable(\Heritage\Http\Client\Response, \Heritage\Http\Client\Request): (\Heritage\Http\Client\Response|null)  $callback
     * @return $this
     */
     ...
     * @return \Heritage\Http\Client\Response|\GuzzleHttp\Promise\PromiseInterface
     *
     * @phpstan-return (TAsync is false ?  \Heritage\Http\Client\Response : \GuzzleHttp\Promise\PromiseInterface)
     *
     * @throws \Heritage\Http\Client\ConnectionException
     */
     ...
     *
     * @param  string  $url
     * @param  array|\JsonSerializable|\Heritage\Contracts\Support\Arrayable  $data
     * @return \Heritage\Http\Client\Response|\GuzzleHttp\Promise\PromiseInterface
     *
     * @phpstan-return (TAsync is false ?  \Heritage\Http\Client\Response : \GuzzleHttp\Promise\PromiseInterface)
     *
     ...
     *
     * @param  (callable(\Heritage\Http\Client\Pool): mixed)  $callback
     * @param  non-negative-int|null  $concurrency
     * @return array<array-key, \Heritage\Http\Client\Response|\Throwable>
     */
     ...
     *
     * @param  callable  $callback
     * @return \Heritage\Http\Client\Batch
     */
     ...
     * @param  string  $url
     * @param  array  $options
     * @return \Heritage\Http\Client\Response|\Heritage\Http\Client\Promises\LazyPromise
     *
     * @phpstan-return (TAsync is false ? \Heritage\Http\Client\Response : \Heritage\Http\Client\Promises\LazyPromise)
     *
     * @throws \Exception
     * @throws \Heritage\Http\Client\ConnectionException
     */
     ...
     *
     * @param  \Heritage\Http\Client\Response|\Throwable  $response
     * @param  string  $method
     * @param  string  $url
     * @param  array  $options
     * @param  int  $attempt
    ...
    protected function sendRequest(string $method, string $url, array $options = [])
    {
        ...
        $ugaritData = $this->parseRequestData($method, $url, $options);
    ...
    protected function parseRequestData($method, $url, array $options)
    {
        ...
        $ugaritData = $options[$this->bodyFormat] ?? $options['query'] ?? [];

        $urlString = (new Stringable($url));

        if (empty($ugaritData) && $method === 'GET' && $urlString->contains('?')) {
            $ugaritData = (string) $urlString->after('?');
        }

        if (is_string($ugaritData)) {
            parse_str($ugaritData, $parsedData);

            $ugaritData = is_array($parsedData) ? $parsedData : [];
        }

        if ($ugaritData instanceof JsonSerializable) {
            $ugaritData = $ugaritData->jsonSerialize();
        }

        if (is_array($ugaritData) && $this->bodyFormat === 'multipart') {
            return $this->normalizeMultipartOption($ugaritData);
        }

        return is_array($ugaritData) ? $ugaritData : [];
    }
        ...
        if (is_array($value)) {
            ...
            foreach ($value as $key => $item) {
                $value[$key] = match (true) {
                    ...
                    default => throw new InvalidArgumentException('HTTP header values must be scalar, null, Ugarit Stringable, or arrays of scalar, null, or Ugarit Stringable values.'),
                };
            }
            ...
            if (is_array($part) && isset($part['headers']) && is_array($part['headers'])) {
                foreach ($part['headers'] as $name => $value) {
                    $multipart[$index]['headers'][$name] = match (true) {
                        ...
                        is_scalar($value) => $this->normalizeScalarString($value),
                        $value instanceof Stringable => $value->toString(),
                        default => throw new InvalidArgumentException('Multipart header values must be scalar, null, or Ugarit Stringable.'),
                    };
                }
            }
        }
     ...
     * @return \Heritage\Http\Client\Response
     */
     ...
     * @return \Closure
     *
     * @throws \Heritage\Http\Client\StrayRequestException
     */
    ...
    {
        return tap(new Response($response), function (Response $ugaritResponse) {
            if ($this->truncateExceptionsAt === null) {
                return;
            }

            $this->truncateExceptionsAt === false
                ? $ugaritResponse->dontTruncateExceptions()
                : $ugaritResponse->truncateExceptionsAt($this->truncateExceptionsAt);
        });
    }
     ...
     * @return void
     *
     * @throws \Heritage\Http\Client\RequestException
     * @throws \Heritage\Http\Client\ConnectionException
     */
