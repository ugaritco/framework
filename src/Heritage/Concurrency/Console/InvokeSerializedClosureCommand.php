...
namespace Heritage\Concurrency\Console;
    ...
    public function handle()
    {
        try {
            $this->output->write(json_encode([
                'successful' => true,
                'result' => serialize($this->ugarit->call(match (true) {
                    ! is_null($this->argument('code')) => unserialize($this->argument('code')),
                    isset($_SERVER['UGARIT_INVOKABLE_CLOSURE']) => unserialize(
                        base64_decode($_SERVER['UGARIT_INVOKABLE_CLOSURE'])
                    ),
                    default => fn () => null,
                })),
            ]));
