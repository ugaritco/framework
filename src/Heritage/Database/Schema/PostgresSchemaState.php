...
class PostgresSchemaState extends SchemaState
{
    ...
    {
        $command = 'pg_restore --no-owner --no-acl --clean --if-exists --host="${:UGARIT_LOAD_HOST}" --port="${:UGARIT_LOAD_PORT}" --username="${:UGARIT_LOAD_USER}" --dbname="${:UGARIT_LOAD_DATABASE}" "${:UGARIT_LOAD_PATH}"';

        if (str_ends_with($path, '.sql')) {
            $command = 'psql --file="${:UGARIT_LOAD_PATH}" --host="${:UGARIT_LOAD_HOST}" --port="${:UGARIT_LOAD_PORT}" --username="${:UGARIT_LOAD_USER}" --dbname="${:UGARIT_LOAD_DATABASE}"';
        }
    ...
    {
        return 'pg_dump --no-owner --no-acl --host="${:UGARIT_LOAD_HOST}" --port="${:UGARIT_LOAD_PORT}" --username="${:UGARIT_LOAD_USER}" --dbname="${:UGARIT_LOAD_DATABASE}"';
    }
    ...
    protected function baseVariables(array $config)
    {
        ...
        $variables['UGARIT_LOAD_DATABASE'] = $config['database'];
