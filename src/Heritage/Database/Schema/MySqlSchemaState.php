...
namespace Heritage\Database\Schema;
...
class MySqlSchemaState extends SchemaState
{
    ...
    public function dump(Connection $connection, $path)
    {
        $this->executeDumpProcess($this->makeProcess(
            $this->baseDumpCommand().' --routines --result-file="${:UGARIT_LOAD_PATH}" --no-data'
        ), $this->output, array_merge($this->baseVariables($this->connection->getConfig()), [
            'UGARIT_LOAD_PATH' => $path,
        ]));
    ...
    public function load($path)
    {
        ...
        $command = 'mysql '.$this->connectionString($versionInfo).' --database="${:UGARIT_LOAD_DATABASE}" < "${:UGARIT_LOAD_PATH}"';
    ...
    protected function baseDumpCommand()
    {
        ...
        return $command.' "${:UGARIT_LOAD_DATABASE}"';
    }
    ...
    {
        $value = ' --user="${:UGARIT_LOAD_USER}" --password="${:UGARIT_LOAD_PASSWORD}"';

        $config = $this->connection->getConfig();

        $value .= $config['unix_socket'] ?? false
            ? ' --socket="${:UGARIT_LOAD_SOCKET}"'
            : ' --host="${:UGARIT_LOAD_HOST}" --port="${:UGARIT_LOAD_PORT}"';

        if (isset($config['options'][Mysql::ATTR_SSL_CA])) {
            $value .= ' --ssl-ca="${:UGARIT_LOAD_SSL_CA}"';
        }

        if (isset($config['options'][Mysql::ATTR_SSL_CERT])) {
            $value .= ' --ssl-cert="${:UGARIT_LOAD_SSL_CERT}"';
        }

        if (isset($config['options'][Mysql::ATTR_SSL_KEY])) {
            $value .= ' --ssl-key="${:UGARIT_LOAD_SSL_KEY}"';
        }
    ...
    protected function baseVariables(array $config)
    {
        ...
        return [
            'UGARIT_LOAD_SOCKET' => $config['unix_socket'] ?? '',
            'UGARIT_LOAD_HOST' => is_array($config['host']) ? $config['host'][0] : $config['host'],
            'UGARIT_LOAD_PORT' => $config['port'] ?? '',
            'UGARIT_LOAD_USER' => $config['username'],
            'UGARIT_LOAD_PASSWORD' => $config['password'] ?? '',
            'UGARIT_LOAD_DATABASE' => $config['database'],
            'UGARIT_LOAD_SSL_CA' => $config['options'][Mysql::ATTR_SSL_CA] ?? '',
            'UGARIT_LOAD_SSL_CERT' => $config['options'][Mysql::ATTR_SSL_CERT] ?? '',
            'UGARIT_LOAD_SSL_KEY' => $config['options'][Mysql::ATTR_SSL_KEY] ?? '',
        ];
    }
