...
namespace Heritage\Database\Query\Grammars;

use Heritage\Database\Query\Builder;
use Heritage\Database\Query\JoinLateralClause;
...
class MySqlGrammar extends Grammar
{
     ...
     *
     * @param  \Heritage\Database\Query\Builder  $query
     * @param  \Heritage\Database\Query\IndexHint  $indexHint
     * @return string
     *
    ...
    protected function compileLegacyGroupLimit(Builder $query)
    {
        ...
        $partition = ', @ugarit_row := if(@ugarit_group = '.$column.', @ugarit_row + 1, 1) as `ugarit_row`';
        $partition .= ', @ugarit_group := '.$column;
        ...
        $from = '(select @ugarit_row := 0, @ugarit_group := 0) as `ugarit_vars`, ('.$sql.') as `ugarit_table`';

        $sql = 'select `ugarit_table`.*'.$partition.' from '.$from.' having `ugarit_row` <= '.$limit;

        if (isset($offset)) {
            $sql .= ' and `ugarit_row` > '.$offset;
        }

        return $sql.' order by `ugarit_row`';
    }
    ...
    public function compileUpsert(Builder $query, array $values, array $uniqueBy, array $update)
    {
        ...
        $columns = (new Collection($update))->map(function ($value, $key) use ($useUpsertAlias) {
            ...
            return $useUpsertAlias
                ? $this->wrap($value).' = '.$this->wrap('ugarit_upsert_alias').'.'.$this->wrap($value)
     ...
     *
     * @param  \Heritage\Database\Query\JoinLateralClause  $join
