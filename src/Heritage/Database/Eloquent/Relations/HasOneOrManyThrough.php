 ...
 * @template TResult
 *
 * @extends \Heritage\Database\Eloquent\Relations\Relation<TRelatedModel, TIntermediateModel, TResult>
 */
abstract class HasOneOrManyThrough extends Relation
{
    ...
    protected function buildDictionary(EloquentCollection $results)
    {
        ...
        foreach ($results as $key => $result) {
            if ($isAssociative) {
                $dictionary[$result->ugarit_through_key][$key] = $result;
            } else {
                $dictionary[$result->ugarit_through_key][] = $result;
            }
        }
     ...
     * @param  mixed  $id
     * @param  array  $columns
     * @return ($id is (\Heritage\Contracts\Support\Arrayable<array-key, mixed>|array<mixed>) ? \Heritage\Database\Eloquent\Collection<int, TRelatedModel> : TRelatedModel|null)
     */
     ...
     * @param  array  $columns
     * @return \Heritage\Database\Eloquent\Collection<int, TRelatedModel>
     */
     ...
     * @param  mixed  $id
     * @param  array  $columns
     * @return ($id is (\Heritage\Contracts\Support\Arrayable<array-key, mixed>|array<mixed>) ? \Heritage\Database\Eloquent\Collection<int, TRelatedModel> : TRelatedModel)
     *
     * @throws \Heritage\Database\Eloquent\ModelNotFoundException<TRelatedModel>
     */
     ...
     *     ? \Heritage\Database\Eloquent\Collection<int, TRelatedModel>|TValue
    ...
    protected function shouldSelect(array $columns = ['*'])
    {
        ...
        return array_merge($columns, [$this->getQualifiedFirstKeyName().' as ugarit_through_key']);
    }
     ...
     *
     * @return \Heritage\Support\LazyCollection<int, TRelatedModel>
     */
