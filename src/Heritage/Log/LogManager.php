...
class LogManager implements LoggerInterface
{
     ...
     *
     * @param  string  $name
     * @param  \Heritage\Log\Logger  $logger
     * @return \Heritage\Log\Logger
     */
    ...
    protected function createEmergencyLogger()
    {
        ...
        $handler = new StreamHandler(
            $config['path'] ?? $this->app->storagePath().'/logs/ugarit.log',
            $this->level(['level' => 'debug'])
        );

        return new Logger(
            new Monolog('ugarit', $this->prepareHandlers([$handler])),
            $this->app['events']
        );
    }
