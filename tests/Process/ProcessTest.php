...
namespace Heritage\Tests\Process;

use Carbon\CarbonInterval;
use Heritage\Contracts\Process\ProcessResult;
use Heritage\Process\Exceptions\ProcessFailedException;
use Heritage\Process\Exceptions\ProcessIdleTimedOutException;
...
class ProcessTest extends TestCase
{
    ...
    public function testFakeAssertionsWithArrayCommands()
    {
        ...
        $factory->run(['php', 'scribe', 'migrate']);

        $factory->assertRan(['php', 'scribe', 'migrate']);
        $factory->assertRanTimes(['php', 'scribe', 'migrate'], 1);
        $factory->assertNotRan(['php', 'scribe', 'migrate:rollback']);
        $factory->assertDidntRun(['php', 'scribe', 'migrate:rollback']);
        $factory->assertRanInOrder([['php', 'scribe', 'migrate']]);
    }
