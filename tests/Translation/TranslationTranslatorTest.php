...
namespace Heritage\Tests\Translation;

use Heritage\Contracts\Translation\Loader;
use Heritage\Support\Carbon;
use Heritage\Support\Collection;
use Heritage\Tests\Translation\Fixtures\Enums\Bar;
use Heritage\Tests\Translation\Fixtures\Enums\Baz;
use Heritage\Tests\Translation\Fixtures\Enums\Foo;
use Heritage\Translation\MessageSelector;
...
class TranslationTranslatorTest extends TestCase
{
    ...
    public function testGetJsonReplacesWithStringable()
    {
        ...
        $t->stringable(function (\Heritage\Support\Carbon $carbon) {
    ...
    public function testGetJsonReplacesWithEnums()
    {
        ...
        $t->getLoader()
            ...
            ->with('en', '*', '*')
            ->andReturn([
                'string_backed_enum' => 'Ugarit 12 was released in :month 2025',
                'int_backed_enum' => 'Stay tuned for Ugarit v:version',
                'unit_enum' => ':person gets excited about every new Ugarit release',
            ]);

        $this->assertSame(
            'Ugarit 12 was released in February 2025',
            $t->get('string_backed_enum', ['month' => Baz::February])
        );

        $this->assertSame(
            'Stay tuned for Ugarit v13',
            $t->get('int_backed_enum', ['version' => Bar::Thirteen])
        );

        $this->assertSame(
            'Hosni gets excited about every new Ugarit release',
    ...
    public function testTagReplacements()
    {
        ...
        $this->assertSame(
            'We have some nice <a href="https://ugarit.com/docs">documentation</a>',
            $t->get(
                'We have some nice <docs-link>documentation</docs-link>',
                [
                    'docs-link' => fn ($children) => "<a href=\"https://ugarit.com/docs\">$children</a>",
                ]
            )
        );
    }
