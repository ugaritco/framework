...
class SupportStringableTest extends TestCase
{
    ...
    {
        $this->assertTrue($this->stringable('https://ugarit.com')->isUrl());
        $this->assertTrue($this->stringable('https://ugarit.com')->isUrl(['https']));

        $this->assertFalse($this->stringable('invalid url')->isUrl());
        $this->assertFalse($this->stringable('https://ugarit.com')->isUrl(['http']));
    }
    ...
    {
        $this->assertTrue($this->stringable('Hello, Ugarit!')->isMatch('/.*,.*!/'));
        $this->assertTrue($this->stringable('Hello, Ugarit!')->isMatch('/^.*$(.*)/'));
        $this->assertTrue($this->stringable('Hello, Ugarit!')->isMatch('/ugarit/i'));
        $this->assertTrue($this->stringable('Hello, Ugarit!')->isMatch('/^(.*(.*(.*)))/'));

        $this->assertFalse($this->stringable('Hello, Ugarit!')->isMatch('/H.o/'));
        $this->assertFalse($this->stringable('Hello, Ugarit!')->isMatch('/^ugarit!/i'));
        $this->assertFalse($this->stringable('Hello, Ugarit!')->isMatch('/ugarit!(.*)/'));
        $this->assertFalse($this->stringable('Hello, Ugarit!')->isMatch('/^[a-zA-Z,!]+$/'));

        $this->assertTrue($this->stringable('Hello, Ugarit!')->isMatch(['/.*,.*!/', '/H.o/']));
        $this->assertTrue($this->stringable('Hello, Ugarit!')->isMatch(['/^ugarit!/i', '/^.*$(.*)/']));
        $this->assertTrue($this->stringable('Hello, Ugarit!')->isMatch(['/ugarit/i', '/ugarit!(.*)/']));
        $this->assertTrue($this->stringable('Hello, Ugarit!')->isMatch(['/^[a-zA-Z,!]+$/', '/^(.*(.*(.*)))/']));
    }
    ...
    {
        $this->assertSame('Ugarit', (string) $this->stringable('ugarit')->ucwords());
        $this->assertSame('Ugarit Framework', (string) $this->stringable('ugarit framework')->ucwords());
        $this->assertSame('Ugarit-Framework', (string) $this->stringable('ugarit-framework')->ucwords('-'));
    ...
    {
        $this->assertSame(' ugarit php framework ', (string) $this->stringable(' ugarit   php  framework ')->deduplicate());
        ...
        $this->assertSame(' ugarit forever ', (string) $this->stringable(' ugaritl    foreverrr  ')->deduplicate([' ', 'l', 'r']));
    }
    ...
    public function testUcsplitOnStringable()
    {
        ...
        $this->assertSame(['Hello', 'From', 'Ugarit'], $this->stringable('HelloFromUgarit')->ucsplit()->toArray());
    ...
    public function testHeadline(): void
    {
        ...
        $this->assertSame('Ugarit Php Framework', (string) $this->stringable('ugarit_php_framework')->headline());
    ...
    {
        $this->assertSame('ugarit', (string) $this->stringable('Ugarit')->lcfirst());
        $this->assertSame('ugarit framework', (string) $this->stringable('Ugarit framework')->lcfirst());
    }

    public function testUcfirst(): void
    {
        $this->assertSame('Ugarit', (string) $this->stringable('ugarit')->ucfirst());
        $this->assertSame('Ugarit framework', (string) $this->stringable('ugarit framework')->ucfirst());
    }
    ...
    {
        $this->assertSame('Ugarit'.PHP_EOL, (string) $this->stringable('Ugarit')->newLine());
    ...
    public function testSquish()
    {
        ...
        $this->assertSame('words with spaces', (string) $this->stringable('
            ...
            spaces
        ')->squish());
        $this->assertSame('ugarit php framework', (string) $this->stringable('   ugarit   php   framework   ')->squish());
    ...
    public function testIsWithMultilineStrings()
    {
        ...
        $this->assertTrue($this->stringable($multilineValue)->is("*namespace Heritage\Tests\*"));
        $this->assertFalse($this->stringable($multilineValue)->is("namespace Heritage\Tests\*"));
        $this->assertFalse($this->stringable($multilineValue)->is("*namespace Heritage\Tests"));
        $this->assertTrue($this->stringable($multilineValue)->is('<?php*'));
        $this->assertTrue($this->stringable($multilineValue)->is("<?php*namespace Heritage\Tests\*"));
    ...
    {
        $this->assertSame('ugarit-php-framework', (string) $this->stringable('UgaritPhpFramework')->kebab());
    }

    public function testChopStart(): void
    {
        $this->assertSame('ugarit.com', (string) $this->stringable('http://ugarit.com')->chopStart('http://'));
        $this->assertSame('http://ugarit.com', (string) $this->stringable('http://ugarit.com')->chopStart('https://'));
        $this->assertSame('ugarit.com', (string) $this->stringable('http://ugarit.com')->chopStart(['https://', 'http://']));
    }
    ...
    {
        $this->assertSame('Ugarit is...',
            (string) $this->stringable('Ugarit is a free, open source PHP web application framework.')->limit(10)
        );
        ...
        $this->assertSame('The PHP', (string) $this->stringable($string)->limit(7, ''));
        $this->assertSame('The PHP framework for web scribes.', (string) $this->stringable($string)->limit(100));
    ...
    {
        $this->assertSame('ugarit_p_h_p_framework', (string) $this->stringable('UgaritPHPFramework')->snake());
        $this->assertSame('ugarit_php_framework', (string) $this->stringable('UgaritPhpFramework')->snake());
        $this->assertSame('ugarit php framework', (string) $this->stringable('UgaritPhpFramework')->snake(' '));
        $this->assertSame('ugarit_php_framework', (string) $this->stringable('Ugarit Php Framework')->snake());
        $this->assertSame('ugarit_php_framework', (string) $this->stringable('Ugarit    Php      Framework   ')->snake());
        // ensure cache keys don't overlap
        $this->assertSame('ugarit__php__framework', (string) $this->stringable('UgaritPhpFramework')->snake('__'));
        $this->assertSame('ugarit_php_framework_', (string) $this->stringable('UgaritPhpFramework_')->snake('_'));
        $this->assertSame('ugarit_php_framework', (string) $this->stringable('ugarit php Framework')->snake());
        $this->assertSame('ugarit_php_frame_work', (string) $this->stringable('ugarit php FrameWork')->snake());
    ...
    {
        $this->assertSame('UgaritPHPFramework', (string) $this->stringable('ugarit_p_h_p_framework')->studly());
        $this->assertSame('UgaritPhpFramework', (string) $this->stringable('ugarit_php_framework')->studly());
        $this->assertSame('UgaritPhPFramework', (string) $this->stringable('ugarit-phP-framework')->studly());
        $this->assertSame('UgaritPhpFramework', (string) $this->stringable('ugarit  -_-  php   -_-   framework   ')->studly());
    ...
    {
        $this->assertSame('UgaritPHPFramework', (string) $this->stringable('ugarit_p_h_p_framework')->pascal());
        $this->assertSame('UgaritPhpFramework', (string) $this->stringable('ugarit_php_framework')->pascal());
        $this->assertSame('UgaritPhPFramework', (string) $this->stringable('ugarit-phP-framework')->pascal());
        $this->assertSame('UgaritPhpFramework', (string) $this->stringable('ugarit  -_-  php   -_-   framework   ')->pascal());
    ...
    {
        $this->assertSame('ugaritPHPFramework', (string) $this->stringable('Ugarit_p_h_p_framework')->camel());
        $this->assertSame('ugaritPhpFramework', (string) $this->stringable('Ugarit_php_framework')->camel());
        $this->assertSame('ugaritPhPFramework', (string) $this->stringable('Ugarit-phP-framework')->camel());
        $this->assertSame('ugaritPhpFramework', (string) $this->stringable('Ugarit  -_-  php   -_-   framework   ')->camel());
    ...
    {
        $this->assertSame(3, $this->stringable('ugaritPHPFramework')->substrCount('a'));
        $this->assertSame(0, $this->stringable('ugaritPHPFramework')->substrCount('z'));
        $this->assertSame(1, $this->stringable('ugaritPHPFramework')->substrCount('l', 2));
        $this->assertSame(0, $this->stringable('ugaritPHPFramework')->substrCount('z', 2));
        $this->assertSame(1, $this->stringable('ugaritPHPFramework')->substrCount('k', -1));
        $this->assertSame(1, $this->stringable('ugaritPHPFramework')->substrCount('k', -1));
        $this->assertSame(1, $this->stringable('ugaritPHPFramework')->substrCount('a', 1, 2));
        $this->assertSame(1, $this->stringable('ugaritPHPFramework')->substrCount('a', 1, 2));
        $this->assertSame(3, $this->stringable('ugaritPHPFramework')->substrCount('a', 1, -2));
        $this->assertSame(1, $this->stringable('ugaritPHPFramework')->substrCount('a', -10, -3));
    }
    ...
    public function testSubstrReplace()
    {
        ...
        $this->assertSame('The Ugarit Framework', (string) $this->stringable('The Framework')->substrReplace('Ugarit ', 4, 0));
        $this->assertSame('Ugarit – The PHP Framework for Web Scribes', (string) $this->stringable('Ugarit Framework')->substrReplace('– The PHP Framework for Web Scribes', 8));
    }
    ...
    public function testJsonSerialize()
    {
        ...
        $this->assertSame('"ugarit-php-framework"', json_encode($this->stringable('UgaritPhpFramework')->kebab()));
        $this->assertSame('["ugarit-php-framework"]', json_encode([$this->stringable('UgaritPhpFramework')->kebab()]));
        $this->assertSame('{"title":"ugarit-php-framework"}', json_encode(['title' => $this->stringable('UgaritPhpFramework')->kebab()]));
    }
    ...
    public function testWordCount()
    {
        ...
        $this->assertEquals(10, $this->stringable('Hi, this is my first contribution to the Ugarit framework.')->wordCount());
    }
    ...
    {
        $sentence = 'Ugarit is a PHP framework. You can access the docs in: {https://ugarit.com/docs}';

        $uri = $this->stringable($sentence)->between('{', '}')->toUri();

        $this->assertInstanceOf(Uri::class, $uri);
        $this->assertSame('https://ugarit.com/docs', (string) $uri);
        $this->assertSame('https://ugarit.com/docs', $uri->toHtml());
    }
