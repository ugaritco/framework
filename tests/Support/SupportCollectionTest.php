...
use Heritage\Support\ItemNotFoundException;
use Heritage\Support\LazyCollection;
use Heritage\Support\MultipleItemsFoundException;
use Heritage\Support\Str;
use Heritage\Support\Stringable;
use Heritage\Tests\Support\Fixtures\TestArrayableObject;
use Heritage\Tests\Support\Fixtures\TestBackedEnum;
use Heritage\Tests\Support\Fixtures\TestEnum;
use Heritage\Tests\Support\Fixtures\TestJsonableObject;
use Heritage\Tests\Support\Fixtures\TestJsonSerializeObject;
use Heritage\Tests\Support\Fixtures\TestJsonSerializeWithScalarValueObject;
use Heritage\Tests\Support\Fixtures\TestStringBackedEnum;
use Heritage\Tests\Support\Fixtures\TestTraversableAndJsonSerializableObject;
...
class SupportCollectionTest extends TestCase
{
    ...
    {
        $duplicates = $collection::make([1, 2, 1, 'ugarit', null, 'ugarit', 'php', null])->duplicates()->all();
        $this->assertSame([2 => 1, 5 => 'ugarit', 7 => null], $duplicates);
        ...
        $duplicates = $collection::make([1, '2', ['ugarit'], ['ugarit'], null, '2'])->duplicates()->all();
        $this->assertSame([3 => ['ugarit'], 5 => '2'], $duplicates);

        // works with mix of objects and primitives **excepts numbers**.
        $expected = new Collection(['ugarit']);
        $duplicates = $collection::make([new Collection(['ugarit']), $expected, $expected, [], '2', '2'])->duplicates()->all();
    ...
    {
        $items = [['framework' => 'vue'], ['framework' => 'ugarit'], ['framework' => 'ugarit']];
        $duplicates = $collection::make($items)->duplicates('framework')->all();
        $this->assertSame([2 => 'ugarit'], $duplicates);
    ...
    {
        $duplicates = $collection::make([1, 2, 1, 'ugarit', null, 'ugarit', 'php', null])->duplicatesStrict()->all();
        $this->assertSame([2 => 1, 5 => 'ugarit', 7 => null], $duplicates);
        ...
        // works with mix of primitives
        $duplicates = $collection::make([1, '2', ['ugarit'], ['ugarit'], null, '2'])->duplicatesStrict()->all();
        $this->assertSame([3 => ['ugarit'], 5 => '2'], $duplicates);

        // works with mix of primitives, objects, and numbers
        $expected = new $collection(['ugarit']);
        $duplicates = $collection::make([new $collection(['ugarit']), $expected, $expected, [], '2', '2'])->duplicatesStrict()->all();
    ...
    public function testReverse($collection)
    {
        ...
        $data = new $collection(['name' => 'taylor', 'framework' => 'ugarit']);
        $reversed = $data->reverse();

        $this->assertSame(['framework' => 'ugarit', 'name' => 'taylor'], $reversed->all());
    }
    ...
    public function testFlip($collection)
    {
        ...
        $data = new $collection(['name' => 'taylor', 'framework' => 'ugarit']);
        $this->assertEquals(['taylor' => 'name', 'ugarit' => 'framework'], $data->flip()->toArray());
    }
    ...
    public function testGroupByAttributeWithStringableKey($collection)
    {
        $data = new $collection($payload = [
            ['name' => new Stringable('Ugarit'), 'url' => '1'],
            ['name' => new HtmlString('Ugarit'), 'url' => '1'],
            ['name' => new class()
            {
        ...
        $this->assertEquals(['Ugarit' => [$payload[0], $payload[1]], 'Framework' => [$payload[2]]], $result->toArray());
    ...
    {
        $c = new $collection([1, 2, 3, 4, 5, 2, 5, 'name' => 'taylor', 'framework' => 'ugarit']);

        $this->assertEquals(1, $c->before(2));
        $this->assertEquals(1, $c->before('2'));
        $this->assertEquals(5, $c->before('taylor'));
        $this->assertSame('taylor', $c->before('ugarit'));
    ...
    {
        $c = new $collection([1, 2, 3, 4, 2, 5, 'name' => 'taylor', 'framework' => 'ugarit']);
        ...
        $this->assertSame('ugarit', $c->after('taylor'));

        $this->assertEquals(4, $c->after(function ($value) {
            return $value > 2;
        }));
        $this->assertSame('ugarit', $c->after(function ($value) {
            return ! is_numeric($value);
        }));
    }
    ...
    {
        $c = new $collection(['name' => 'taylor', 'framework' => 'ugarit']);
        $this->assertEquals(['name', 'framework'], $c->keys()->all());

        $c = new $collection(['taylor', 'ugarit']);
