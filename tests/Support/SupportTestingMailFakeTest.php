<?php

namespace Heritage\Tests\Support;

use Heritage\Bus\Queueable;
use Heritage\Contracts\Queue\ShouldQueue;
use Heritage\Contracts\Translation\HasLocalePreference;
use Heritage\Mail\Mailable;
use Heritage\Mail\MailManager;
use Heritage\Support\Testing\Fakes\MailFake;
use Mockery;
use PHPUnit\Framework\ExpectationFailedException;
use PHPUnit\Framework\TestCase;

class SupportTestingMailFakeTest extends TestCase
{
    /**
     * @var \Mockery
     */
    private $mailManager;

    /**
     * @var \Heritage\Support\Testing\Fakes\MailFake
     */
    private $fake;

    /**
     * @var \Heritage\Tests\Support\MailableStub
     */
    private $mailable;

    protected function setUp(): void
    {
        $this->mailManager = Mockery::mock(MailManager::class, function ($mock) {
            $mock->expects('getDefaultDriver')
                ->andReturn('smtp');
        });
        $this->fake = new MailFake($this->mailManager);
        $this->mailable = new MailableStub;
    }

    public function testAssertSent()
    {
        try {
            $this->fake->assertSent(MailableStub::class);
            $this->fail();
        } catch (ExpectationFailedException $e) {
            $this->assertStringContainsString('The expected [Heritage\Tests\Support\MailableStub] mailable was not sent.', $e->getMessage());
        }

        $this->fake->to('taylor@ugarit.com')->send($this->mailable);

        $this->fake->assertSent(MailableStub::class);
    }

    public function testAssertSentTo()
    {
        try {
            $this->fake->assertSent(MailableStub::class, 'taylor@ugarit.com');
            $this->fail();
        } catch (ExpectationFailedException $e) {
            $this->assertStringContainsString('The expected [Heritage\Tests\Support\MailableStub] mailable was not sent to address [taylor@ugarit.com].', $e->getMessage());
        }

        $this->fake->to('taylor@ugarit.com')->send($this->mailable);

        $this->fake->assertSent(MailableStub::class, 'taylor@ugarit.com');
    }

    public function testAssertSentToMultiple()
    {
        $this->fake->to('dries@ugarit.com')->send($this->mailable);
        $this->fake->to('taylor@ugarit.com')->send($this->mailable);

        $this->fake->to(['nuno@ugarit.com', 'jess@ugarit.com'])->send($this->mailable);

        $this->fake->assertSent(MailableStub::class, 3);
        $this->fake->assertSent(
            MailableStub::class,
            ['taylor@ugarit.com', 'dries@ugarit.com', 'nuno@ugarit.com', 'jess@ugarit.com']
        );
    }

    public function testAssertSentWhenRecipientHasPreferredLocale()
    {
        $user = new LocalizedRecipientStub;

        $this->fake->to($user)->send($this->mailable);

        $this->fake->assertSent(MailableStub::class, function ($mail) use ($user) {
            return $mail->hasTo($user) && $mail->locale === 'au';
        });
    }

    public function testAssertTo()
    {
        $this->fake->to('taylor@ugarit.com')->send($this->mailable);

        $this->fake->assertSent(MailableStub::class, function ($mail) {
            return $mail->hasTo('taylor@ugarit.com');
        });
    }

    public function testAssertCc()
    {
        $this->fake->cc('taylor@ugarit.com')->send($this->mailable);

        $this->fake->assertSent(MailableStub::class, function ($mail) {
            return $mail->hasCc('taylor@ugarit.com');
        });
    }

    public function testAssertBcc()
    {
        $this->fake->bcc('taylor@ugarit.com')->send($this->mailable);

        $this->fake->assertSent(MailableStub::class, function ($mail) {
            return $mail->hasBcc('taylor@ugarit.com');
        });
    }

    public function testAssertNotSent()
    {
        $this->fake->assertNotSent(MailableStub::class);

        $this->fake->to('taylor@ugarit.com')->send($this->mailable);

        try {
            $this->fake->assertNotSent(MailableStub::class);
            $this->fail();
        } catch (ExpectationFailedException $e) {
            $this->assertStringContainsString('The unexpected [Heritage\Tests\Support\MailableStub] mailable was sent.', $e->getMessage());
        }
    }

    public function testAssertNotSentWithClosure()
    {
        $callback = function (MailableStub $mail) {
            return $mail->hasTo('taylor@ugarit.com');
        };

        $this->fake->assertNotSent($callback);

        $this->fake->to('taylor@ugarit.com')->send($this->mailable);

        $this->expectException(ExpectationFailedException::class);
        $this->expectExceptionMessageMatches('/The unexpected \['.preg_quote(MailableStub::class, '/').'\] mailable was sent./m');

        $this->fake->assertNotSent($callback);
    }

    public function testAssertNotSentWithString()
    {
        $this->fake->assertNotSent(MailableStub::class, 'taylor@ugarit.com');

        $this->fake->to('taylor@ugarit.com')->send($this->mailable);

        $this->expectExceptionObject(new ExpectationFailedException('The unexpected ['.MailableStub::class.'] mailable was sent to address [taylor@ugarit.com].'));

        $this->fake->assertNotSent(MailableStub::class, 'taylor@ugarit.com');
    }

    public function testAssertNotSentWithArray()
    {
        $this->fake->assertNotSent(MailableStub::class, ['taylor@ugarit.com', 'dries@ugarit.com']);

        $this->fake->to('dries@ugarit.com')->send($this->mailable);

        $this->expectExceptionObject(new ExpectationFailedException('The unexpected ['.MailableStub::class.'] mailable was sent to address [dries@ugarit.com].'));

        $this->fake->assertNotSent(MailableStub::class, ['taylor@ugarit.com', 'dries@ugarit.com']);
    }

    public function testAssertSentTimes()
    {
        $this->fake->to('taylor@ugarit.com')->send($this->mailable);
        $this->fake->to('taylor@ugarit.com')->send($this->mailable);

        try {
            $this->fake->assertSent(MailableStub::class, 1);
            $this->fail();
        } catch (ExpectationFailedException $e) {
            $this->assertStringContainsString('The expected [Heritage\Tests\Support\MailableStub] mailable was sent 2 times instead of 1 time.', $e->getMessage());
        }

        $this->fake->assertSent(MailableStub::class, 2);
    }

    public function testAssertSentCount()
    {
        $this->fake->to('taylor@ugarit.com')->send($this->mailable);
        $this->fake->to('taylor@ugarit.com')->send($this->mailable);

        try {
            $this->fake->assertSentCount(1);
            $this->fail();
        } catch (ExpectationFailedException $e) {
            $this->assertStringContainsString('The total number of mailables sent was 2 instead of 1.', $e->getMessage());
        }

        $this->fake->assertSentCount(2);
    }

    public function testAssertQueued()
    {
        try {
            $this->fake->assertQueued(MailableStub::class);
            $this->fail();
        } catch (ExpectationFailedException $e) {
            $this->assertStringContainsString('The expected [Heritage\Tests\Support\MailableStub] mailable was not queued.', $e->getMessage());
        }

        $this->fake->to('taylor@ugarit.com')->queue($this->mailable);

        $this->fake->assertQueued(MailableStub::class);
    }

    public function testAssertQueuedTo()
    {
        try {
            $this->fake->assertQueued(MailableStub::class, 'taylor@ugarit.com');
            $this->fail();
        } catch (ExpectationFailedException $e) {
            $this->assertStringContainsString('The expected [Heritage\Tests\Support\MailableStub] mailable was not queued to address [taylor@ugarit.com].', $e->getMessage());
        }

        $this->fake->to('taylor@ugarit.com')->queue($this->mailable);

        $this->fake->assertQueued(MailableStub::class, 'taylor@ugarit.com');
    }

    public function testAssertQueuedToMultiple()
    {
        $this->fake->to('dries@ugarit.com')->queue($this->mailable);
        $this->fake->to('taylor@ugarit.com')->queue($this->mailable);

        $this->fake->to(['nuno@ugarit.com', 'jess@ugarit.com'])->queue($this->mailable);

        $this->fake->assertQueued(MailableStub::class, 3);
        $this->fake->assertQueued(
            MailableStub::class,
            ['taylor@ugarit.com', 'dries@ugarit.com', 'nuno@ugarit.com', 'jess@ugarit.com']
        );
    }

    public function testAssertQueuedTimes()
    {
        $this->fake->to('taylor@ugarit.com')->queue($this->mailable);
        $this->fake->to('taylor@ugarit.com')->queue($this->mailable);

        try {
            $this->fake->assertQueued(MailableStub::class, 1);
            $this->fail();
        } catch (ExpectationFailedException $e) {
            $this->assertStringContainsString('The expected [Heritage\Tests\Support\MailableStub] mailable was queued 2 times instead of 1 time.', $e->getMessage());
        }

        $this->fake->assertQueued(MailableStub::class, 2);
    }

    public function testAssertSentOnce()
    {
        $this->fake->to('taylor@ugarit.com')->send($this->mailable);

        $this->fake->assertSentOnce(MailableStub::class);

        $this->fake->to('taylor@ugarit.com')->send($this->mailable);

        try {
            $this->fake->assertSentOnce(MailableStub::class);
            $this->fail();
        } catch (ExpectationFailedException $e) {
            $this->assertStringContainsString('The expected [Heritage\Tests\Support\MailableStub] mailable was sent 2 times instead of 1 time.', $e->getMessage());
        }
    }

    public function testAssertQueuedOnce()
    {
        $this->fake->to('taylor@ugarit.com')->queue($this->mailable);

        $this->fake->assertQueuedOnce(MailableStub::class);

        $this->fake->to('taylor@ugarit.com')->queue($this->mailable);

        try {
            $this->fake->assertQueuedOnce(MailableStub::class);
            $this->fail();
        } catch (ExpectationFailedException $e) {
            $this->assertStringContainsString('The expected [Heritage\Tests\Support\MailableStub] mailable was queued 2 times instead of 1 time.', $e->getMessage());
        }
    }

    public function testAssertQueuedTimesCalledDirectly()
    {
        $this->fake->to('taylor@ugarit.com')->queue($this->mailable);
        $this->fake->to('taylor@ugarit.com')->queue($this->mailable);

        try {
            $this->fake->assertQueuedTimes(MailableStub::class, 1);
            $this->fail();
        } catch (ExpectationFailedException $e) {
            $this->assertStringContainsString('The expected [Heritage\Tests\Support\MailableStub] mailable was queued 2 times instead of 1 time.', $e->getMessage());
        }

        $this->fake->assertQueuedTimes(MailableStub::class, 2);
    }

    public function testAssertNotQueuedWithString()
    {
        $this->fake->assertNotQueued(MailableStub::class, 'taylor@ugarit.com');

        $this->fake->to('taylor@ugarit.com')->queue($this->mailable);

        $this->expectExceptionObject(new ExpectationFailedException('The unexpected ['.MailableStub::class.'] mailable was queued to address [taylor@ugarit.com].'));

        $this->fake->assertNotQueued(MailableStub::class, 'taylor@ugarit.com');
    }

    public function testAssertNotQueuedWithArray()
    {
        $this->fake->assertNotQueued(MailableStub::class, ['taylor@ugarit.com', 'dries@ugarit.com']);

        $this->fake->to('dries@ugarit.com')->queue($this->mailable);

        $this->expectExceptionObject(new ExpectationFailedException('The unexpected ['.MailableStub::class.'] mailable was queued to address [dries@ugarit.com].'));

        $this->fake->assertNotQueued(MailableStub::class, ['taylor@ugarit.com', 'dries@ugarit.com']);
    }

    public function testAssertQueuedCount()
    {
        $this->fake->to('taylor@ugarit.com')->queue($this->mailable);
        $this->fake->to('taylor@ugarit.com')->queue($this->mailable);

        try {
            $this->fake->assertQueuedCount(1);
            $this->fail();
        } catch (ExpectationFailedException $e) {
            $this->assertStringContainsString('The total number of mailables queued was 2 instead of 1.', $e->getMessage());
        }

        $this->fake->assertQueuedCount(2);
    }

    public function testSendQueuesAMailableThatShouldBeQueued()
    {
        $this->fake->to('taylor@ugarit.com')->send(new QueueableMailableStub);

        $this->fake->assertQueued(QueueableMailableStub::class);

        try {
            $this->fake->assertSent(QueueableMailableStub::class);
            $this->fail();
        } catch (ExpectationFailedException $e) {
            $this->assertStringContainsString('The expected [Heritage\Tests\Support\QueueableMailableStub] mailable was not sent.', $e->getMessage());
        }
    }

    public function testAssertNothingSent()
    {
        $this->fake->assertNothingSent();

        $this->fake->to('taylor@ugarit.com')->send($this->mailable);

        try {
            $this->fake->assertNothingSent();
            $this->fail();
        } catch (ExpectationFailedException $e) {
            $this->assertStringContainsString("The following mailables were sent unexpectedly:\n\n- Heritage\Tests\Support\MailableStub", $e->getMessage());
        }
    }

    public function testAssertNothingQueued()
    {
        $this->fake->assertNothingQueued();

        $this->fake->to('taylor@ugarit.com')->queue($this->mailable);

        try {
            $this->fake->assertNothingQueued();
            $this->fail();
        } catch (ExpectationFailedException $e) {
            $this->assertStringContainsString("The following mailables were queued unexpectedly:\n\n- Heritage\Tests\Support\MailableStub", $e->getMessage());
        }
    }

    public function testAssertOutgoingCount()
    {
        $this->fake->assertNothingOutgoing();

        $this->fake->to('taylor@ugarit.com')->queue($this->mailable);

        try {
            $this->fake->assertOutgoingCount(2);
            $this->fail();
        } catch (ExpectationFailedException $e) {
            $this->assertStringContainsString('The total number of outgoing mailables was 1 instead of 2.', $e->getMessage());
        }

        $this->fake->to('taylor@ugarit.com')->send($this->mailable);

        $this->fake->assertOutgoingCount(2);
    }

    public function testAssertQueuedWithClosure()
    {
        $this->fake->to($user = new LocalizedRecipientStub)->queue($this->mailable);

        $this->fake->assertQueued(function (MailableStub $mail) use ($user) {
            return $mail->hasTo($user);
        });
    }

    public function testAssertSentWithClosure()
    {
        $this->fake->to($user = new LocalizedRecipientStub)->send($this->mailable);

        $this->fake->assertSent(function (MailableStub $mail) use ($user) {
            return $mail->hasTo($user);
        });
    }

    public function testMissingMethodsAreForwarded()
    {
        $this->mailManager->expects('foo')->andReturn('bar');

        $this->assertSame('bar', $this->fake->foo());
    }

    public function testAssertMailer()
    {
        $this->fake->to('taylor@ugarit.com')->send($this->mailable);

        $this->fake->assertSent(MailableStub::class, function ($mail) {
            return $mail->usesMailer('smtp');
        });

        $this->fake->mailer('ses')->to('taylor@ugarit.com')->send($this->mailable);

        $this->fake->assertSent(MailableStub::class, function ($mail) {
            return $mail->usesMailer('ses');
        });

        $this->fake->mailer('sendgrid')->to('taylor@ugarit.com')->queue($this->mailable);

        $this->fake->assertQueued(MailableStub::class, function ($mail) {
            return $mail->usesMailer('sendgrid');
        });

        $this->fake->mailer('mailjet')->to('taylor@ugarit.com')->queue($this->mailable);

        $this->fake->assertQueued(MailableStub::class, function ($mail) {
            return $mail->usesMailer('mailjet');
        });
    }

    public function testDriverMethod()
    {
        $this->fake->driver('ses')->to('taylor@ugarit.com')->send($this->mailable);

        $this->fake->assertSent(MailableStub::class, function ($mail) {
            return $mail->hasTo('taylor@ugarit.com') &&
                $mail->usesMailer('ses');
        });

        $this->fake->driver('sendgrid')->to('taylor@ugarit.com')->queue($this->mailable);

        $this->fake->assertQueued(MailableStub::class, function ($mail) {
            return $mail->hasTo('taylor@ugarit.com') &&
                $mail->usesMailer('sendgrid');
        });

        $this->fake->driver('mailjet')->to('taylor@ugarit.com')->queue($this->mailable);

        $this->fake->assertQueued(MailableStub::class, function ($mail) {
            return $mail->hasTo('taylor@ugarit.com') &&
                $mail->usesMailer('mailjet');
        });
    }

    public function testQueueSetsQueueOnMailable()
    {
        $mailable1 = new QueueableMailableStub;
        $this->fake->queue($mailable1, 'podcasts');

        $this->fake->assertQueued(QueueableMailableStub::class, function ($mail) {
            return $mail->queue === 'podcasts';
        });

        $mailable2 = new QueueableMailableStub;
        $this->fake->queue($mailable2, MailTestQueueEnum::Emails);

        $this->fake->assertQueued(QueueableMailableStub::class, function ($mail) {
            return $mail->queue === 'emails';
        });

        $mailable3 = new QueueableMailableStub;
        $this->fake->onQueue('podcasts', $mailable3);

        $this->fake->assertQueued(QueueableMailableStub::class, function ($mail) {
            return $mail->queue === 'podcasts';
        });

        $mailable4 = new QueueableMailableStub;
        $this->fake->queueOn('videos', $mailable4);

        $this->fake->assertQueued(QueueableMailableStub::class, function ($mail) {
            return $mail->queue === 'videos';
        });
    }

    public function testLaterSetsQueueOnMailable()
    {
        $mailable1 = new QueueableMailableStub;
        $this->fake->later(15, $mailable1, 'podcasts');

        $this->fake->assertQueued(QueueableMailableStub::class, function ($mail) {
            return $mail->queue === 'podcasts';
        });

        $mailable2 = new QueueableMailableStub;
        $this->fake->laterOn('videos', 45, $mailable2);

        $this->fake->assertQueued(QueueableMailableStub::class, function ($mail) {
            return $mail->queue === 'videos';
        });
    }
}

enum MailTestQueueEnum: string
{
    case Emails = 'emails';
}

class MailableStub extends Mailable
{
    public $framework = 'Ugarit';

    protected $version = '6.0';

    /**
     * Build the message.
     *
     * @return $this
     */
    public function build()
    {
        $this->with('first_name', 'Taylor')
            ->withLastName('Otwell');
    }
}

class QueueableMailableStub extends Mailable implements ShouldQueue
{
    use Queueable;

    public $framework = 'Ugarit';

    protected $version = '6.0';

    /**
     * Build the message.
     *
     * @return $this
     */
    public function build()
    {
        $this->with('first_name', 'Taylor')
            ->withLastName('Otwell');
    }
}

class LocalizedRecipientStub implements HasLocalePreference
{
    public $email = 'taylor@ugarit.com';

    public function preferredLocale()
    {
        return 'au';
    }
}
