...
class DevCommandTest extends TestCase
{
    protected function setUp(): void
    {
        ...
        $app['env'] = 'testing';
        $app['config'] = new Repository(['app' => ['name' => 'Ugarit']]);
    }
    ...
    {
        $this->assertSame(
            "@ugarit/multiplex --title 'scribe dev · my-app' "
                ."'server@#93c5fd,php scribe serve' 'vite@#fcd34d,npm run dev'",
    ...
    public function testMultiplexCommandKeepsColonsInLabels()
    {
        $command = $this->command()->buildMultiplexCommandForTesting([
            ['name' => 'queue:work', 'command' => 'php scribe queue:work', 'source' => [], 'color' => '#c4b5fd'],
        ]);

        $this->assertStringContainsString("'queue:work@#c4b5fd,php scribe queue:work'", $command);
    }
    ...
    {
        $this->assertSame(
            'concurrently -c "#93c5fd,#fcd34d" "php scribe serve" "npm run dev" --names=server,vite'
    ...
    {
        return [
            ['name' => 'server', 'command' => 'php scribe serve', 'source' => [], 'color' => '#93c5fd'],
