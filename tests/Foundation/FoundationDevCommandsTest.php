...
use Heritage\Foundation\DevCommand;
use Heritage\Foundation\DevCommandColor;
use Heritage\Foundation\DevCommandMode;
...
class FoundationDevCommandsTest extends TestCase
{
    ...
    public function testScribePrefixesCommand()
    {
        DevCommands::scribe('serve --host=localhost', 'server');

        $commands = DevCommands::commands();

        $this->assertSame('php scribe serve --host=localhost', $commands[0]['command']);
        $this->assertSame('server', $commands[0]['name']);
    }

    public function testScribeDerivesNameFromCommand()
    {
        DevCommands::scribe('queue:listen --tries=1');
    ...
    public function testResolvePriorityDetectsUserlandThroughDevCommandsFrame()
    {
        ...
        $trace = [
            ...
            ['file' => base_path('app/Providers/AppServiceProvider.php'), 'line' => 19, 'function' => 'scribe', 'class' => DevCommands::class],
            ['file' => base_path('vendor/ugarit/framework/src/Heritage/Foundation/Application.php'), 'line' => 896, 'function' => 'register', 'class' => 'App\\Providers\\AppServiceProvider'],
        ];
    ...
    public function testResolvePriorityDetectsVendor()
    {
        ...
        $trace = [
            ...
            ['file' => base_path('vendor/ugarit/framework/src/Heritage/Foundation/Application.php'), 'line' => 896, 'function' => 'register', 'class' => 'Some\\Package\\ServiceProvider'],
        ];
    ...
    public function testRegisterDefaultsRegistersExpectedCommands()
    {
        ...
        $provider = Mockery::mock('alias:Ugarit\Pail\PailServiceProvider');
        ...
        $this->assertSame('php scribe serve', collect($commands)->firstWhere('name', 'server')['command']);
    ...
    public function testWithoutVendorCommandsExcludesOnlyVendorCommands()
    {
        $ref = new ReflectionClass(DevCommands::class);
        $ref->getProperty('commands')->setValue(null, [
            'server' => new DevCommand('php scribe serve', [], 'server', DevCommand::PRIORITY_DEFAULT),
            'vendor' => new DevCommand('echo vendor', [], 'vendor', DevCommand::PRIORITY_VENDOR),
        ]);
