...
use Heritage\Database\LostConnectionDetector;
use Heritage\Foundation\Cloud\AgentAwareLostConnectionDetector;
use Heritage\Foundation\Cloud\AgentUnreachableException;
use Heritage\Foundation\Cloud\CloudJob;
use Heritage\Foundation\Cloud\Events;
use Heritage\Foundation\Cloud\FailedJobProvider;
use Heritage\Foundation\Cloud\ManagedQueueNotFoundException;
use Heritage\Foundation\Cloud\Queue;
use Heritage\Foundation\Cloud\QueueConnector;
use Heritage\Foundation\CloudBootstrapper;
...
use Heritage\Queue\Connectors\ConnectorInterface;
use Heritage\Queue\Connectors\SqsConnector;
use Heritage\Queue\Events\WorkerStopping;
use Heritage\Queue\Failed\FileFailedJobProvider;
use Heritage\Queue\Jobs\FakeJob;
use Heritage\Queue\Jobs\SqsJob;
use Heritage\Queue\SqsQueue;
use Heritage\Queue\Worker;
use Heritage\Queue\WorkerStopReason;
...
class QueueTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $_SERVER['UGARIT_CLOUD'] = '1';
        $_SERVER['UGARIT_CLOUD_MANAGED_QUEUES_CONFIG'] = json_encode([
            'driver' => 'cloud',
            'connection' => [
                'driver' => 'sqs',
                'region' => 'us-east-2',
        ...
        $this->app['config']->set('queue.connections.cloud', json_decode($_SERVER['UGARIT_CLOUD_MANAGED_QUEUES_CONFIG'], true));
    }
    ...
    protected function tearDown(): void
    {
        ...
        unset($_SERVER['UGARIT_CLOUD'], $_SERVER['UGARIT_CLOUD_MANAGED_QUEUES_CONFIG']);
        Worker::$restartable = true;
        Worker::$pausable = true;
        Worker::$memoryExceededExitCode = null;
    }
    ...
    {
        $argv = $_SERVER['argv'];
        $_SERVER['argv'] = ['scribe', 'queue:work'];
    ...
    public function testItConfiguresCloudConnectionFromManagedQueuesConfig()
    {
        ...
        $expected = json_decode($_SERVER['UGARIT_CLOUD_MANAGED_QUEUES_CONFIG'], true);
    ...
    {
        unset($_SERVER['UGARIT_CLOUD_MANAGED_QUEUES_CONFIG']);
        $this->app['config']->set('queue.connections.cloud', null);
    ...
    public function testItRemembersTheQueueForTheProcessedEvent()
    {
        ...
        // alongside each pop.
        $_SERVER['argv'] = ['scribe', 'queue:work', '--queue=first'];
        $queue->pop('first');
        $_SERVER['argv'] = ['scribe', 'queue:work', '--queue=second'];
        $queue->pop('second');
        $_SERVER['argv'] = ['scribe', 'queue:work', '--queue=third'];
        $queue->pop('third');
    ...
    public function testPopReceivesFromTheAgentForANonDefaultWorkerQueue()
    {
        ...
        $_SERVER['argv'] = ['scribe', 'queue:work', '--queue=emails'];
    ...
    public function testPopReceivesFromSqsForAMultiQueueWorker()
    {
        ...
        $_SERVER['argv'] = ['scribe', 'queue:work', '--queue=high,low'];
    ...
    public function testFindGetsUrlAndDecryptsResponse()
    {
        ...
        Http::fake([
            'https://cloud.ugarit.com/*' => Http::response($encrypted),
        ]);

        $result = $provider->find('https://cloud.ugarit.com/api/jobs/test-job-id?signature=abc');
        ...
        Http::assertSent(fn ($request) => $request->url() === 'https://cloud.ugarit.com/api/jobs/test-job-id?signature=abc');
    }
    ...
    public function testFindReturnsNullWhenDecryptionFails()
    {
        ...
        Http::fake([
            'https://cloud.ugarit.com/*' => Http::response('not-valid-encrypted-data'),
        ]);

        try {
            $provider->find('https://cloud.ugarit.com/api/jobs/test-job-id?signature=abc');
            $this->fail();
        } catch (Throwable $e) {
    ...
    public function testFindReturnsNullWhenHttpRequestFails()
    {
        ...
        Http::fake([
            'https://cloud.ugarit.com/*' => Http::response('Server Error', 500),
        ]);
    ...
    public function testForgetEmitsEventAfterFind()
    {
        ...
        $url = 'https://cloud.ugarit.com/api/jobs/forget-test-id?signature=abc';
        $provider->find($url);
    ...
    public function testForgetReturnsFalseWithoutPriorFind()
    {
        ...
        $result = $provider->forget('https://cloud.ugarit.com/api/jobs/some-id?signature=abc');
