...
use Closure;
use Heritage\Cache\DatabaseStore;
...
class CacheDatabaseStoreTest extends TestCase
{
    ...
    public function testNullIsReturnedAndItemDeletedWhenItemIsExpired()
    {
        ...
        $deleteQuery->expects('whereIn')->with('key', ['prefixfoo', 'prefixheritage:cache:flexible:created:foo'])->andReturn($deleteQuery);
    ...
    public function testItemsMayBeRemovedFromCache()
    {
        ...
        $table = Mockery::mock(Builder::class);
        $store->getConnection()->expects('table')->with('table')->andReturn($table);
        $table->expects('whereIn')->with('key', ['prefixfoo', 'prefixheritage:cache:flexible:created:foo'])->andReturn($table);
    ...
    {
        $store = $this->getStore();
        $connection = Mockery::mock(\Heritage\Database\ConnectionInterface::class);
