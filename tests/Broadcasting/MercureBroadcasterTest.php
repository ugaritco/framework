...
use Heritage\Broadcasting\Mercure\ChannelEncrypter;
...
{
    /**
     * @var \Heritage\Broadcasting\Broadcasters\MercureBroadcaster
     */
    public $broadcaster;
    ...
    public function testAuthDeduplicatesRequestedChannels()
    {
        ...
        $this->assertSame([['match' => 'https://ugarit.alt/echo/channel/private-room.1']], $claims['authorization_details'][0]['topics']);
        $this->assertSame([['match' => 'https://ugarit.alt/echo/whisper/private-room.1']], $claims['authorization_details'][1]['topics']);
    }
    ...
    public function testAuthStillMintsATokenWithNoGrantsWhenEveryChannelIsPublic()
    {
        ...
        $this->assertSame('https://ugarit.alt/echo/', $response->getData(true)['topic_prefix']);
    ...
    public function testAuthFlagsADeniedChannelAndKeepsTheGrantedOnes()
    {
        ...
        $this->assertSame([['match' => 'https://ugarit.alt/echo/channel/private-room.1']], $details[0]['topics']);
        $this->assertSame([['match' => 'https://ugarit.alt/echo/whisper/private-room.1']], $details[1]['topics']);
    }
    ...
    public function testAuthExcludesPublicChannelsAndGrantsOnlyGuardedOnes()
    {
        ...
        $this->assertSame([
            [
                ...
                'actions' => ['subscribe'],
                'topics' => [['match' => 'https://ugarit.alt/echo/channel/private-room.1']],
            ],
            [
                'type' => 'https://mercure.rocks/authorization-detail',
                'actions' => ['subscribe', 'publish'],
                'topics' => [['match' => 'https://ugarit.alt/echo/whisper/private-room.1']],
            ],
    ...
    public function testAuthGrantsPresenceChannelWithPayloadAndSubscriptionEventsPattern()
    {
        ...
        $this->assertSame([
            ['match' => 'https://ugarit.alt/echo/channel/presence-room.1'],
            [
                'match' => '/.well-known/mercure/subscriptions/:match_type/https%3A%2F%2Fugarit.alt%2Fecho%2Fchannel%2Fpresence-room.1{/:subscriber}?',
    ...
    public function testBroadcastPublishesOneUnprivatedUpdateWhenEveryChannelIsPublic()
    {
        $this->hub->shouldReceive('publish')->once()->with(m::on(function (Update $update) {
            ...
            return $update->getTopics() === ['https://ugarit.alt/echo/channel/news', 'https://ugarit.alt/echo/channel/weather']
                && ! $update->isPrivate()
    ...
    public function testBroadcastPublishesOnePrivateUpdatePerGuardedChannel()
    {
        foreach (['private-room.1', 'presence-room.2'] as $channel) {
            $this->hub->shouldReceive('publish')->once()->with(m::on(function (Update $update) use ($channel) {
                ...
                return $update->getTopics() === ['https://ugarit.alt/echo/channel/'.$channel]
                    && $update->isPrivate()
    ...
    {
        $this->hub->shouldReceive('publish')->once()->with(m::on(function (Update $update) {
            return $update->getTopics() === ['https://ugarit.alt/echo/channel/news'] && ! $update->isPrivate();
        }));

        $this->hub->shouldReceive('publish')->once()->with(m::on(function (Update $update) {
            return $update->getTopics() === ['https://ugarit.alt/echo/channel/private-room.1'] && $update->isPrivate();
        }));
    ...
    {
        $this->hub->shouldReceive('publish')->once()->with(m::on(function (Update $update) {
            return $update->getTopics() === ['https://ugarit.alt/echo/channel/order%2F1%20%2A%27%28%29%21'];
        }));
    ...
    public function testAuthGrantsEachPresenceChannelItsOwnPayload()
    {
        ...
        $this->assertSame([['match' => 'https://ugarit.alt/echo/channel/private-inbox.42']], $details[0]['topics']);
        $this->assertArrayNotHasKey('payload', $details[0]);

        $this->assertSame([
            ['match' => 'https://ugarit.alt/echo/channel/presence-room.1'],
            ['match' => '/.well-known/mercure/subscriptions/:match_type/https%3A%2F%2Fugarit.alt%2Fecho%2Fchannel%2Fpresence-room.1{/:subscriber}?', 'match_type' => 'urlpattern'],
        ], $details[1]['topics']);
        $this->assertSame(['user_id' => '42', 'user_info' => ['id' => 42, 'room' => '1']], $details[1]['payload']);

        $this->assertSame([
            ['match' => 'https://ugarit.alt/echo/channel/presence-room.2'],
            ['match' => '/.well-known/mercure/subscriptions/:match_type/https%3A%2F%2Fugarit.alt%2Fecho%2Fchannel%2Fpresence-room.2{/:subscriber}?', 'match_type' => 'urlpattern'],
        ], $details[2]['topics']);
        $this->assertSame(['user_id' => '42', 'user_info' => ['id' => 42, 'room' => '2']], $details[2]['payload']);

        $this->assertSame(['subscribe', 'publish'], $details[3]['actions']);
        $this->assertSame([
            ['match' => 'https://ugarit.alt/echo/whisper/presence-room.1'],
            ['match' => 'https://ugarit.alt/echo/whisper/presence-room.2'],
            ['match' => 'https://ugarit.alt/echo/whisper/private-inbox.42'],
        ], $details[3]['topics']);
    ...
    public function testAuthGrantsEncryptedChannelsInTheSharedPrivateGrant()
    {
        ...
        $this->assertSame([
            [
                ...
                'actions' => ['subscribe'],
                'topics' => [
                    ['match' => 'https://ugarit.alt/echo/channel/private-room.1'],
                    ['match' => 'https://ugarit.alt/echo/channel/private-encrypted-orders.1'],
                ],
            ],
            [
                'type' => 'https://mercure.rocks/authorization-detail',
                'actions' => ['subscribe', 'publish'],
                'topics' => [
                    ['match' => 'https://ugarit.alt/echo/whisper/private-room.1'],
                    ['match' => 'https://ugarit.alt/echo/whisper/private-encrypted-orders.1'],
                ],
            ],
        ], $claims['authorization_details']);
    }
    ...
    public function testBroadcastPublishesOnePrivateUpdatePerEncryptedChannel()
    {
        ...
        $this->hub->shouldReceive('publish')->once()->with(m::on(
            fn (Update $update) => $update->getTopics() === ['https://ugarit.alt/echo/channel/news'] && ! $update->isPrivate()
        ));
        $this->hub->shouldReceive('publish')->once()->with(m::on(
            fn (Update $update) => $update->getTopics() === ['https://ugarit.alt/echo/channel/private-room.1'] && $update->isPrivate()
        ));
