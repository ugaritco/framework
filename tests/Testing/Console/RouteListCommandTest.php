...
namespace Heritage\Tests\Testing\Console;
...
class RouteListCommandTest extends TestCase
{
    ...
    public function testDisplayRoutesForCli()
    {
        ...
        $this->scribe(RouteListCommand::class);
    ...
    public function testDisplayRoutesForCliInVerboseMode()
    {
        ...
        $this->scribe(RouteListCommand::class, ['-v' => true]);
        $output = Scribe::output();
    ...
    public function testRouteCanBeFilteredByName()
    {
        ...
        $this->scribe(RouteListCommand::class, ['--name' => 'foo']);
        $output = Scribe::output();
    ...
    public function testRouteCanBeFilteredByAction()
    {
        ...
        $this->scribe(RouteListCommand::class, ['--action' => 'FooController'])
            ->assertSuccessful()
            ->expectsOutput('')
            ->expectsOutput(
                '  GET|HEAD       foo/{user} Heritage\Tests\Testing\Console\FooController@show'
            )->expectsOutput('')
            ->expectsOutput(
                '                                                              Showing [1] routes'
            )
    ...
    public function testClosurePathIsIncludedInJsonOutput()
    {
        ...
        $this->scribe(RouteListCommand::class, ['--json' => true])
            ->assertSuccessful()
    ...
    public function testDisplayRoutesExceptVendor()
    {
        ...
        $this->scribe(RouteListCommand::class, ['-v' => true, '--except-vendor' => true])
            ->assertSuccessful()
            ->expectsOutput('')
            ->expectsOutput('  GET|HEAD       foo/{user} Heritage\Tests\Testing\Console\FooController@show')
            ->expectsOutput('  ANY            redirect .... Heritage\Routing\RedirectController')
    ...
    //         ->assertSuccessful()
    //         ->expectsOutput('')
    //         ->expectsOutput('  GET|HEAD       users/{user:name} Heritage\Tests\Testing\Console\FooController@show')
