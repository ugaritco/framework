...
namespace Heritage\Tests\Events;

use Heritage\Bus\DebounceLock;
use Heritage\Bus\Dispatcher as BusDispatcher;
use Heritage\Cache\ArrayStore;
use Heritage\Cache\Repository;
use Heritage\Container\Container;
use Heritage\Contracts\Cache\Lock;
...
class QueuedEventsTest extends TestCase
{
    ...
    public function testUniqueLockKeyUsesListenerClassName()
    {
        ...
        $this->assertSame(
            'ugarit_unique_job:'.hash('xxh128', TestDispatcherShouldBeUnique::class).':test-id',
            \Heritage\Bus\UniqueLock::getKey($listener)
        );
    }
    ...
    public function testUniqueLockIsAcquiredWithListenerClassName()
    {
        ...
        $expectedKey = 'ugarit_unique_job:'.hash('xxh128', TestDispatcherShouldBeUnique::class).':unique-listener-id';
    ...
    public function testUniqueViaUsesListenerCacheRepository()
    {
        ...
        $expectedKey = 'ugarit_unique_job:'.hash('xxh128', TestDispatcherShouldBeUniqueWithCustomCache::class).':unique-listener-id';
    ...
    public function testUniqueUntilProcessingLockIsReleasedBeforeHandling()
    {
        ...
        TestDispatcherShouldBeUniqueUntilProcessing::$expectedLockKey = 'ugarit_unique_job:'.hash('xxh128', TestDispatcherShouldBeUniqueUntilProcessing::class).':until-processing-id';
        ...
        $expectedKey = 'ugarit_unique_job:'.hash('xxh128', TestDispatcherShouldBeUniqueUntilProcessing::class).':until-processing-id';
    ...
    public function testQueuePropagatesDebounceOptions()
    {
        ...
        $expectedKey = 'ugarit_debounced_job:'.hash('xxh128', TestDispatcherDebouncedHandler::class).':event-123';
    ...
    public function testExplicitListenerDelayTakesPrecedenceOverDebounceDelay()
    {
        ...
        $queue->expects('laterOn')->with(null, 20, Mockery::on(function ($job) use ($cache) {
            $expectedKey = 'ugarit_debounced_job:'.hash('xxh128', TestDispatcherDebouncedHandlerWithDelay::class).':event-123';
