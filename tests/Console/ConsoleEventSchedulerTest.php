...
class ConsoleEventSchedulerTest extends TestCase
{
    ...
    public function testCommandCreatesNewScribeCommand()
    {
        $schedule = $this->schedule;
        ...
        $scribeBinary = Application::scribeBinary();
        $this->assertEquals($phpBinary.' '.$scribeBinary.' queue:listen', $events[0]->command);
        $this->assertEquals($phpBinary.' '.$scribeBinary.' queue:listen --tries=3', $events[1]->command);
        $this->assertEquals($phpBinary.' '.$scribeBinary.' queue:listen --tries=3', $events[2]->command);
    }

    public function testCreateNewScribeCommandUsingCommandClass()
    {
        $schedule = $this->schedule;
        ...
        $phpBinary = Application::phpBinary();
        $scribeBinary = Application::scribeBinary();
        $this->assertEquals($phpBinary.' '.$scribeBinary.' foo:bar --force', $events[0]->command);
    }

    public function testCreateNewScribeCommandUsingCommandClassObject()
    {
