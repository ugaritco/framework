...
use Heritage\Contracts\Hashing\Hasher;
use Heritage\Contracts\Translation\Translator as TranslatorContract;
use Heritage\Contracts\Validation\DataAwareRule;
use Heritage\Contracts\Validation\ImplicitRule;
...
use Heritage\Validation\DatabasePresenceVerifierInterface;
use Heritage\Validation\Rule as ValidationRule;
use Heritage\Validation\Rules\Exists;
use Heritage\Validation\Rules\ProhibitedIf;
use Heritage\Validation\Rules\Unique;
use Heritage\Validation\ValidationData;
...
class ValidationValidatorTest extends TestCase
{
    ...
    public function testCapitalizedDisplayableValuesAreReplaced()
    {
        ...
        $v = new Validator($trans, ['url' => 'ugarit.com'], ['url' => 'starts_with:http,https']);
        $this->assertFalse($v->passes());
    ...
    public function testCustomExceptionMustExtendValidationException()
    {
        ...
        $this->expectExceptionObject(new InvalidArgumentException('Exception [RuntimeException] is invalid. It must extend [Heritage\Validation\ValidationException].'));
    ...
    {
        $trans = $this->getHeritageArrayTranslator();
        $v = new Validator($trans, ['value' => base64_encode('Ugarit')], ['value' => 'base64']);
        $this->assertTrue($v->passes());
    ...
    public function testValidateEndsWith()
    {
        ...
        $v = new Validator($trans, ['url' => 'ugarit.com'], ['url' => 'ends_with:http']);
        $this->assertFalse($v->passes());
        $this->assertSame('The url must end with one of the following values http', $v->messages()->first('url'));

        $trans = $this->getHeritageArrayTranslator();
        $trans->addLines(['validation.ends_with' => 'The :attribute must end with one of the following values :values'], 'en');
        $v = new Validator($trans, ['url' => 'ugarit.com'], ['url' => 'ends_with:http,https']);
        $this->assertFalse($v->passes());
    ...
    public function testValidateStartsWith()
    {
        ...
        $trans = $this->getHeritageArrayTranslator();
        $trans->addLines(['validation.starts_with' => 'The :attribute must start with one of the following values :values'], 'en');
        $v = new Validator($trans, ['url' => 'ugarit.com'], ['url' => 'starts_with:http']);
        $this->assertFalse($v->passes());
    ...
    {
        $v = new Validator($this->getHeritageArrayTranslator(), ['x' => 'foo@gmäil.com'], ['x' => 'email']);
        $this->assertTrue($v->passes());
    }

    public function testValidateEmailWithStrictCheck()
    {
        $v = new Validator($this->getHeritageArrayTranslator(), ['x' => 'foo@bar '], ['x' => 'email:strict']);
        $this->assertFalse($v->passes());
    }

    public function testValidateEmailWithFilterCheck()
    {
        $v = new Validator($this->getHeritageArrayTranslator(), ['x' => 'foo@bar'], ['x' => 'email:filter']);
        $this->assertFalse($v->passes());

        $v = new Validator($this->getHeritageArrayTranslator(), ['x' => 'example@example.com'], ['x' => 'email:filter']);
        $this->assertTrue($v->passes());

        // Unicode characters are not allowed
        $v = new Validator($this->getHeritageArrayTranslator(), ['x' => 'exämple@example.com'], ['x' => 'email:filter']);
        $this->assertFalse($v->passes());

        $v = new Validator($this->getHeritageArrayTranslator(), ['x' => 'exämple@exämple.com'], ['x' => 'email:filter']);
        $this->assertFalse($v->passes());
    }

    public function testValidateEmailWithFilterUnicodeCheck()
    {
        $v = new Validator($this->getHeritageArrayTranslator(), ['x' => 'foo@bar'], ['x' => 'email:filter_unicode']);
        $this->assertFalse($v->passes());

        $v = new Validator($this->getHeritageArrayTranslator(), ['x' => 'example@example.com'], ['x' => 'email:filter_unicode']);
        $this->assertTrue($v->passes());

        // Any unicode characters are allowed only in local-part
        $v = new Validator($this->getHeritageArrayTranslator(), ['x' => 'exämple@example.com'], ['x' => 'email:filter_unicode']);
        $this->assertTrue($v->passes());

        $v = new Validator($this->getHeritageArrayTranslator(), ['x' => 'exämple@exämple.com'], ['x' => 'email:filter_unicode']);
        $this->assertFalse($v->passes());
    }
    ...
    public function testValidateEmailWithCustomClassCheck()
    {
        ...
        $v = new Validator($this->getHeritageArrayTranslator(), ['x' => 'foo@bar '], ['x' => 'email:'.NoRFCWarningsValidation::class]);
        $v->setContainer($container);
    ...
    public function testValidateUrlWithProtocols()
    {
        ...
        $v = new Validator($trans, ['x' => 'http://ugarit.com'], ['x' => 'url:https']);
        $this->assertFalse($v->passes());
    ...
    public static function validUrls()
    {
        return [
            ...
            ['https://ugarit.com?'],
            ['https://ugarit.com?q=1'],
            ['https://ugarit.com/?q=1'],
            ['https://ugarit.com#'],
            ['https://ugarit.com#fragment'],
            ['https://ugarit.com/#fragment'],
            ['https://domain1'],
            ['https://domain12/'],
...
1',
    ...
    public function testItemAwareSometimesAddingRules()
    {
        ...
        $trans = $this->getHeritageArrayTranslator();
        $v = new Validator($trans, ['attendee' => ['name' => 'Taylor', 'title' => 'Creator of Ugarit', 'type' => 'Developer']], ['attendee.*' => 'string']);
    ...
    public function testCustomValidationObject()
    {
        // Test passing case...
        $v = new Validator(
            $this->getHeritageArrayTranslator(),
            ['name' => 'taylor'],
            [
    ...
    public function testValidateReturnsValidatedData()
    {
        ...
        $v = new Validator($this->getHeritageArrayTranslator(), $post, ['first' => 'required', 'preferred' => 'required']);
    ...
    public function testValidateReturnsValidatedDataNestedRules()
    {
        ...
        $v = new Validator($this->getHeritageArrayTranslator(), $post, $rules);
        $v->sometimes('type', 'required', function () {
            return false;
        });
        $data = $v->validate();
    ...
    public function testValidateReturnsValidatedDataNestedChildRules()
    {
        ...
        $v = new Validator($this->getHeritageArrayTranslator(), $post, ['nested.foo' => 'required']);
    ...
    public function testValidateReturnsValidatedDataNestedArrayRules()
    {
        ...
        $v = new Validator($this->getHeritageArrayTranslator(), $post, ['nested.*.bar' => 'required']);
        $v->sometimes('nested.*.type', 'required', function () {
            return false;
        });
        $data = $v->validate();
    ...
    public function testValidateFailsWithAsterisksAsDataKeys()
    {
        ...
        $validator = new Validator($this->getHeritageArrayTranslator(), $post, $rules);
    ...
    public function getHeritageArrayTranslator()
    {
