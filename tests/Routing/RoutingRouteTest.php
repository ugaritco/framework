...
use Heritage\Auth\Middleware\Authorize;
...
use Heritage\Routing\CallableDispatcher;
...
use Heritage\Routing\ControllerDispatcher;
use Heritage\Routing\Events\PreparingResponse;
use Heritage\Routing\Events\ResponsePrepared;
use Heritage\Routing\Events\Routing;
use Heritage\Routing\Exceptions\UrlGenerationException;
use Heritage\Routing\Middleware\SubstituteBindings;
use Heritage\Routing\ResourceRegistrar;
use Heritage\Routing\Route;
use Heritage\Routing\RouteCollection;
use Heritage\Routing\RouteGroup;
use Heritage\Routing\Router;
use Heritage\Routing\UrlGenerator;
use Heritage\Support\Str;
use Heritage\Tests\Routing\Fixtures\CategoryBackedEnum;
use LogicException;
...
class RoutingRouteTest extends TestCase
{
    ...
    {
        $this->expectExceptionObject(new ModelNotFoundException('No query results for model [Heritage\Tests\Routing\RouteModelBindingNullStub].'));
    ...
    {
        $this->expectExceptionObject(new UnexpectedValueException('Invalid route action: [Heritage\Tests\Routing\RouteTestControllerStub].'));
    ...
    public function testImplicitBindingsWithOptionalParameterUsingEnumIsAlwaysCastedToEnum()
    {
        ...
        $router->get('foo/{bar?}', [
            'middleware' => SubstituteBindings::class,
            'uses' => function (?\Heritage\Tests\Routing\Fixtures\CategoryBackedEnum $bar = null) {
    ...
    {
        $this->expectExceptionObject(new UrlGenerationException('Missing required parameter for [Route: ugarit_route_redirect_destination] [URI: users/{user}] [Missing parameter: user].'));
    ...
    public function testRouteCanMiddlewareCanBeAssigned()
    {
        ...
        $this->assertEquals([
            'foo',
            'can:create,Heritage\Routing\Route',
        ], $route->middleware());
    ...
    public function testRouteDeserializationAllowedClasses()
    {
        ...
        $serializedClosure = serialize(\Ugarit\SerializableClosure\SerializableClosure::unsigned($closureWithUse));
