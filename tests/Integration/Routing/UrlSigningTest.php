...
class UrlSigningTest extends TestCase
{
    ...
    public function testItCanGenerateMiddlewareDefinitionViaStaticMethod()
    {
        ...
        $this->assertSame('Heritage\Routing\Middleware\ValidateSignature:relative', $signature);

        $signature = (string) ValidateSignature::absolute();
        $this->assertSame('Heritage\Routing\Middleware\ValidateSignature', $signature);

        $signature = (string) ValidateSignature::relative(['foo', 'bar']);
        $this->assertSame('Heritage\Routing\Middleware\ValidateSignature:relative,foo,bar', $signature);

        $signature = (string) ValidateSignature::absolute(['foo', 'bar']);
        $this->assertSame('Heritage\Routing\Middleware\ValidateSignature:foo,bar', $signature);
    }
