...
class ScheduleListCommandTest extends TestCase
{
    ...
    {
        $this->withoutMockingConsoleOutput()->scribe(ScheduleListCommand::class, ['--json' => true]);
    ...
    public function testDisplaySchedule()
    {
        ...
        $this->scribe(ScheduleListCommand::class)
            ->assertSuccessful()
            ->expectsOutput('  0 0     1 1-12/3 *  php scribe foo:command .... Next Due: 3 months from now')
            ->expectsOutput('  0 14,18 * *      *  php scribe inspire ........ Next Due: 14 hours from now')
            ->expectsOutput('  * *     * *      *  php scribe foobar a='.ProcessUtils::escapeArgument('b').' ... Next Due: 1 minute from now')
            ->expectsOutput('  * *     * *      *  Heritage\Tests\Integration\Console\Scheduling\FooJob  Next Due: 1 minute from now')
            ->expectsOutput('  * *     * *      *  Heritage\Tests\Integration\Console\Scheduling\FooParamJob  Next Due: 1 minute from now')
            ->expectsOutput('  * *     * *      *  foo-named-job .............. Next Due: 1 minute from now')
            ->expectsOutput('  * *     * *      *  foo-named-param-job ........ Next Due: 1 minute from now')
            ->expectsOutput('  0 9,17  * *      *  php scribe inspire ......... Next Due: 9 hours from now')
            ->expectsOutput('  0 10    * *      *  php scribe inspire ........ Next Due: 10 hours from now')
            ->expectsOutput('  * *     * *      *  Heritage\Tests\Integration\Console\Scheduling\FooCall  Next Due: 1 minute from now')
            ->expectsOutput('  * *     * *      *  Closure at: Heritage\Tests\Integration\Console\Scheduling\FooCall::fooFunction  Next Due: 1 minute from now')
    ...
    public function testDisplayScheduleAsJson()
    {
        ...
        $this->assertSame('php scribe foo:command', $data[0]['command']);
        ...
        $this->assertSame('php scribe foobar a='.ProcessUtils::escapeArgument('b'), $data[2]['command']);
        $this->assertNull($data[2]['description']);
        $this->assertSame('1 minute from now', $data[2]['next_due_date_human']);

        $this->assertSame('Heritage\Tests\Integration\Console\Scheduling\FooJob', $data[3]['command']);
    ...
    public function testDisplayScheduleWithEnvironmentFilterAsJson()
    {
        ...
        $this->withoutMockingConsoleOutput()->scribe(ScheduleListCommand::class, [
            '--environment' => 'production',
            '--json' => true,
        ]);
        ...
        $this->assertSame('Heritage\Tests\Integration\Console\Scheduling\FooJob', $data[1]['command']);
    ...
    public function testDisplayScheduleWithMultipleEnvironmentFilterAsJson()
    {
        ...
        $this->withoutMockingConsoleOutput()
            ->scribe('schedule:list --environment=staging --environment=local --json');
        ...
        $this->assertSame('php scribe foobar a='.ProcessUtils::escapeArgument('b'), $data[0]['command']);
        $this->assertSame(['staging', 'local'], $data[0]['environments']);

        $this->assertSame('*/5 * * * *', $data[1]['expression']);
        $this->assertSame('php scribe inspire', $data[1]['command']);
        $this->assertSame(['local'], $data[1]['environments']);

        $this->assertSame('*/10 * * * *', $data[2]['expression']);
        $this->assertSame('Heritage\Tests\Integration\Console\Scheduling\FooJob', $data[2]['command']);
    ...
    public function testDisplayScheduleWithSortAsJson()
    {
        ...
        $this->assertSame('php scribe foo:command', $data[2]['command']);
    }
    ...
    public function testDisplayScheduleAsJsonWithTimezone()
    {
        ...
        $this->assertSame('php scribe inspire', $data[0]['command']);
    }
    ...
    public function testExpressionTimezoneConversion($expression, $eventTimezone, $displayTimezone, $expectedExpressions)
    {
        ...
        $this->withoutMockingConsoleOutput()->scribe(ScheduleListCommand::class, $options);
        $output = Scribe::output();
    ...
    public function testExpressionTimezoneConversionUsesOffsetAtNextRunDate()
    {
        ...
        $data = json_decode(Scribe::output(), true);
    ...
    public function testDisplayScheduleAsJsonInVerboseMode()
    {
        ...
        $this->assertSame('0 0 1 1-12/3 *', $data[0]['expression']);
        $this->assertSame(Application::phpBinary().' '.Application::scribeBinary().' foo:command', $data[0]['command']);
        $this->assertSame('This is the description of the command.', $data[0]['description']);

        $this->assertSame('* * * * *', $data[1]['expression']);
        $this->assertSame(Application::phpBinary().' '.Application::scribeBinary().' inspire', $data[1]['command']);
    ...
    public function testDisplayScheduleWithSort()
    {
        ...
        $this->scribe(ScheduleListCommand::class, ['--next' => true])
            ->assertSuccessful()
            ->expectsOutput('  * *     * *      *  php scribe foobar a='.ProcessUtils::escapeArgument('b').' ... Next Due: 1 minute from now')
            ->expectsOutput('  * *     * *      *  Heritage\Tests\Integration\Console\Scheduling\FooJob  Next Due: 1 minute from now')
            ->expectsOutput('  * *     * *      *  Heritage\Tests\Integration\Console\Scheduling\FooParamJob  Next Due: 1 minute from now')
    ...
    public function testDisplayScheduleInVerboseMode()
    {
        ...
        $this->scribe(ScheduleListCommand::class, ['-v' => true])
            ->assertSuccessful()
    ...
    public function testDisplayScheduleSubMinute()
    {
        ...
        $this->scribe(ScheduleListCommand::class)
            ->assertSuccessful()
            ->expectsOutput('  * 0 * * 0 1s   php scribe inspire ............. Next Due: 1 second from now')
            ->expectsOutput('  * * * * * 2s   php scribe inspire ............ Next Due: 2 seconds from now')
            ->expectsOutput('  * * * * * 5s   php scribe inspire ............ Next Due: 5 seconds from now')
            ->expectsOutput('  * * * * * 10s  php scribe inspire ........... Next Due: 10 seconds from now')
            ->expectsOutput('  * * * * * 15s  php scribe inspire ........... Next Due: 15 seconds from now')
            ->expectsOutput('  * * * * * 20s  php scribe inspire ........... Next Due: 20 seconds from now')
            ->expectsOutput('  * * * * * 30s  php scribe inspire ........... Next Due: 30 seconds from now');
    }
    ...
    public function testClosureCommandsMayBeScheduled()
    {
        ...
        Scribe::command('one', $closure)->weekly()->everySecond();
        Scribe::command('two', $closure)->everyTwoSeconds();
        Scribe::command('three', $closure)->everyFiveSeconds();
        Scribe::command('four', $closure)->everyTenSeconds();
        Scribe::command('five', $closure)->everyFifteenSeconds();
        Scribe::command('six', $closure)->everyTwentySeconds()->hourly();
        Scribe::command('six', $closure)->everyThreeHours()->everySecond();

        $this->scribe(ScheduleListCommand::class)
            ->assertSuccessful()
            ->expectsOutput('  * 0   * * 0 1s   php scribe one ............... Next Due: 1 second from now')
            ->expectsOutput('  * *   * * * 2s   php scribe two .............. Next Due: 2 seconds from now')
            ->expectsOutput('  * *   * * * 5s   php scribe three ............ Next Due: 5 seconds from now')
            ->expectsOutput('  * *   * * * 10s  php scribe four ............ Next Due: 10 seconds from now')
            ->expectsOutput('  * *   * * * 15s  php scribe five ............ Next Due: 15 seconds from now')
            ->expectsOutput('  0 *   * * * 20s  php scribe six ............. Next Due: 20 seconds from now')
            ->expectsOutput('  * */3 * * * 1s   php scribe six ............... Next Due: 1 second from now');
    }
