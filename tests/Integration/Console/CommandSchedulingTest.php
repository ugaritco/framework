...
class CommandSchedulingTest extends TestCase
{
    ...
    /**
     * Just in case Testbench starts to ship an `scribe` script, we'll check and save a backup.
     *
     * @var string|null
     */
    protected $originalScribe;
    ...
    protected function setUp(): void
    {
        ...
        $this->writeScribeScript();
    }

    protected function tearDown(): void
    {
        $this->fs->delete($this->logfile);
        $this->fs->delete(base_path('scribe'));

        if (! is_null($this->originalScribe)) {
            $this->fs->put(base_path('scribe'), $this->originalScribe);
        }
    ...
    protected function writeScribeScript()
    {
        $path = base_path('scribe');

        // Save existing scribe script if there is one
        if ($this->fs->exists($path)) {
            $this->originalScribe = $this->fs->get($path);
        }
...
// This is a custom scribe script made specifically for:
//
// {$thisFile}
//
...
class CommandSchedulingTestCommand_{$this->id} extends Heritage\Console\Command
{
    ...
    {
        \$logfile = {$logfile};
        (new Heritage\Filesystem\Filesystem)->append(\$logfile, "handled\\n");
    }
}

// Register command with Kernel
Heritage\Console\Application::starting(function (\$scribe) {
    \$scribe->add(new CommandSchedulingTestCommand_{$this->id});
});

// Add command to scheduler so that the after() callback is trigger in our spawned process
Heritage\Foundation\Application::getInstance()
    ->booted(function (\$app) {
        \$app->resolving(Heritage\Console\Scheduling\Schedule::class, function(\$schedule) {
            \$fs = new Heritage\Filesystem\Filesystem;
