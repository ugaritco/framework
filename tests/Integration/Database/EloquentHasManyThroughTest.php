...
namespace Heritage\Tests\Integration\Database\EloquentHasManyThroughTest;
...
class EloquentHasManyThroughTest extends DatabaseTestCase
{
    ...
    public function testBasicCreateAndRetrieve()
    {
        ...
        $this->assertEquals(
            $mate1->refresh()->getAttributes() + ['ugarit_through_key' => '1'],
        ...
        $this->assertEquals(
            $mate2->refresh()->getAttributes() + ['ugarit_through_key' => '1'],
            $result->getAttributes()
        );
    ...
    public function testGlobalScopeColumns()
    {
        ...
        $this->assertEquals(['id' => 2, 'ugarit_through_key' => 1], $teamMates[0]->getAttributes());
    ...
    public function testCreateOrFirstWhenRecordDoesntExist()
    {
        ...
        $article = $team->articles()->createOrFirst(
            ['title' => 'Ugarit Forever'],
            ['user_id' => $tony->id],
        );

        $this->assertTrue($article->wasRecentlyCreated);
        $this->assertSame('Ugarit Forever', $article->title);
    ...
    public function testCreateOrFirstWhenRecordExists()
    {
        ...
        $this->assertSame('Ugarit Forever', $newArticle->title);
    ...
    public function testCreateOrFirstRegressionIssue()
    {
        ...
        $existingTaylorArticle = $taylor->articles()->create(['title' => 'Ugarit Forever']);
        ...
        $this->assertSame('Ugarit Forever', $newArticle->refresh()->title);
        $this->assertTrue($taylor->is($newArticle->user));

        $this->assertSame('Ugarit Forever', $existingTaylorArticle->refresh()->title);
    ...
    {
        // On Ugarit 10.21.0, a bug was introduced that would update the wrong model when using `updateOrCreate()`,
