...
class SchemaBuilderSchemaNameTest extends DatabaseTestCase
{
    protected function setUp(): void
    {
        ...
        if ($this->usesSqliteInMemoryDatabaseConnection()) {
            $this->markTestSkipped('Test cannot be run using :memory: database connection, SQLite test file is here: \Heritage\Tests\Integration\Database\Sqlite\SchemaBuilderSchemaNameTest');
        }
    }
    ...
    public function testSchemas($connection)
    {
        ...
        $this->assertEqualsCanonicalizing(
            match ($this->driver) {
                'mysql', 'mariadb' => ['ugarit', 'my_schema'],
    ...
    public function testForeignKeys($connection)
    {
        ...
        $schema->create('my_schema.table', function (Blueprint $table) {
            ...
            $table->foreignId('my_table_id')
                ->constrained(table: in_array($this->driver, ['mariadb', 'mysql']) ? 'ugarit.my_tables' : null);
        });
        $schema->create('table', function (Blueprint $table) {
    ...
    public function testComment($connection)
    {
        ...
        $tableName = $connection === 'with-prefix' ? 'example_table' : 'table';
        $defaultSchema = $this->driver === 'pgsql' ? 'public' : 'ugarit';
    ...
    public function testHasTable($connection)
    {
        ...
        } catch(\Heritage\Database\QueryException) {
            //
        }
