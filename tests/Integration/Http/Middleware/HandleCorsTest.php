...
namespace Heritage\Tests\Integration\Http\Middleware;

use Heritage\Contracts\Http\Kernel;
use Heritage\Foundation\Validation\ValidatesRequests;
...
class HandleCorsTest extends TestCase
{
    ...
    {
        $this->app['config']->set('cors.allowed_origins', ['*.ugarit.com']);

        $crawler = $this->call('OPTIONS', 'api/ping', [], [], [], [
            'HTTP_ORIGIN' => 'http://test.ugarit.com',
            'HTTP_ACCESS_CONTROL_REQUEST_METHOD' => 'POST',
        ]);

        $this->assertSame('http://test.ugarit.com', $crawler->headers->get('Access-Control-Allow-Origin'));
        $this->assertEquals(204, $crawler->getStatusCode());
    }
    ...
    public function testOriginsWildcardIncludesNestedSubdomains()
    {
        ...
        $crawler = $this->call('OPTIONS', 'api/ping', [], [], [], [
            'HTTP_ORIGIN' => 'http://api.service.test.ugarit.com',
            'HTTP_ACCESS_CONTROL_REQUEST_METHOD' => 'POST',
        ]);

        $this->assertSame('http://api.service.test.ugarit.com', $crawler->headers->get('Access-Control-Allow-Origin'));
        $this->assertEquals(204, $crawler->getStatusCode());
    }
