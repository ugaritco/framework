...
use Heritage\Cache\RateLimiting\GlobalLimit;
...
use Heritage\Http\Exceptions\ThrottleRequestsException;
use Heritage\Http\Request;
use Heritage\Routing\Exceptions\MissingRateLimiterException;
use Heritage\Routing\Middleware\ThrottleRequests;
use Heritage\Support\Carbon;
use Heritage\Support\Facades\Cache;
use Heritage\Support\Facades\RateLimiter as RateLimiterFacade;
...
class ThrottleRequestsTest extends TestCase
{
    ...
    public function testItCanGenerateDefinitionViaStaticMethod()
    {
        ...
        $this->assertSame('Heritage\Routing\Middleware\ThrottleRequests:gold-tier', $signature);

        $signature = (string) ThrottleRequests::with(25);
        $this->assertSame('Heritage\Routing\Middleware\ThrottleRequests:25', $signature);

        $signature = (string) ThrottleRequests::with(25, 2);
        $this->assertSame('Heritage\Routing\Middleware\ThrottleRequests:25,2', $signature);

        $signature = (string) ThrottleRequests::with(25, 2, 'foo');
        $this->assertSame('Heritage\Routing\Middleware\ThrottleRequests:25,2,foo', $signature);
        ...
        $this->assertSame('Heritage\Routing\Middleware\ThrottleRequests:60,1,foo', $signature);
    }
