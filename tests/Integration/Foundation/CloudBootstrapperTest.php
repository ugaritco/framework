...
{
    #[WithConfig('database.connections.pgsql', ['host' => 'test-pooler.pg.ugarit.cloud', 'username' => 'test-username', 'password' => 'test-password'])]
    public function test_it_can_resolve_core_container_aliases()
    {
        CloudBootstrapper::configureUnpooledPostgresConnection($this->app);

        $this->assertEquals([
            'host' => 'test.pg.ugarit.cloud',
    ...
    {
        $_SERVER['UGARIT_CLOUD_DISK_CONFIG'] = json_encode(
            [
                [
                    'disk' => 'test-disk',
        ...
        unset($_SERVER['UGARIT_CLOUD_DISK_CONFIG']);
    }
    ...
    public function test_it_does_not_override_a_different_filesystem_disk()
    {
        ...
        unset($_SERVER['UGARIT_CLOUD_DISK_CONFIG'], $_SERVER['FILESYSTEM_DISK']);
    }
    ...
    public function test_it_respects_log_levels()
    {
        ...
        $this->assertSame('notice', $this->app['config']->get('logging.channels.ugarit-cloud-socket.level'));
    ...
    public function test_it_configures_a_cloud_logging_socket_timeout()
    {
        ...
        $this->assertSame(2.0, $this->app['config']->get('logging.channels.ugarit-cloud-socket.with.timeout'));
    }
    ...
    public function test_it_aliases_cloud_logging_channel()
    {
        ...
        $this->assertSame(
            $this->app['config']->get('logging.channels.ugarit-cloud-socket'),
