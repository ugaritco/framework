...
use Heritage\Database\ConnectionResolverInterface as Resolver;
...
use Heritage\Database\Eloquent\Casts\ArrayObject;
use Heritage\Database\Eloquent\Casts\AsArrayObject;
use Heritage\Database\Eloquent\Casts\AsCollection;
use Heritage\Database\Eloquent\Casts\AsEncryptedArrayObject;
use Heritage\Database\Eloquent\Casts\AsEncryptedCollection;
use Heritage\Database\Eloquent\Casts\AsEnumArrayObject;
use Heritage\Database\Eloquent\Casts\AsEnumCollection;
use Heritage\Database\Eloquent\Casts\AsFluent;
use Heritage\Database\Eloquent\Casts\AsHtmlString;
use Heritage\Database\Eloquent\Casts\AsStringable;
use Heritage\Database\Eloquent\Casts\AsUri;
...
use Heritage\Database\Eloquent\JsonEncodingException;
use Heritage\Database\Eloquent\MassAssignmentException;
use Heritage\Database\Eloquent\MissingAttributeException;
use Heritage\Database\Eloquent\Model;
use Heritage\Database\Eloquent\Relations\BelongsTo;
use Heritage\Database\Eloquent\Relations\Relation;
use Heritage\Database\Query\Builder as BaseBuilder;
use Heritage\Database\Query\Grammars\Grammar;
use Heritage\Database\Query\Processors\Processor;
use Heritage\Support\Carbon;
use Heritage\Support\Collection as BaseCollection;
...
use Heritage\Tests\Database\Fixtures\TestCast;
use Heritage\Tests\Database\Fixtures\TestValueObject;
...
class DatabaseEloquentModelTest extends TestCase
{
    ...
    public function testOnly()
    {
        ...
        $this->assertEquals(['project' => 'ugarit'], $model->only('project'));
    ...
    public function testExcept()
    {
        ...
        $this->assertEquals(['project' => 'ugarit'], $model->except('first_name', 'last_name'));
        $this->assertEquals(['project' => 'ugarit'], $model->except(['first_name', 'last_name']));
    }
    ...
    {
        $model = $this->getMockBuilder('Heritage\Tests\Database\EloquentDateModelStub')->onlyMethods(['getDateFormat'])->getMock();
    ...
    {
        $events = Mockery::mock(Dispatcher::class);
        $events->expects('listen')->with('eloquent.creating: Heritage\Tests\Database\EloquentModelStub', EloquentTestObserverStub::class.'@creating');
        $events->expects('listen')->with('eloquent.saved: Heritage\Tests\Database\EloquentModelStub', EloquentTestObserverStub::class.'@saved');
    ...
    public function testModelObserversCanBeAttachedToModelsWithStringUsingAttribute()
    {
        $events = Mockery::mock(Dispatcher::class);
        $events->expects('dispatch')->times(2);
        $events->expects('listen')->with('eloquent.creating: Heritage\Tests\Database\EloquentModelWithObserveAttributeStub', EloquentTestObserverStub::class.'@creating');
        $events->expects('listen')->with('eloquent.saved: Heritage\Tests\Database\EloquentModelWithObserveAttributeStub', EloquentTestObserverStub::class.'@saved');
        $events->shouldReceive('forget');
    ...
    public function testModelObserversCanBeAttachedToModelsThroughAnArrayUsingAttribute()
    {
        $events = Mockery::mock(Dispatcher::class);
        $events->expects('dispatch')->times(2);
        $events->expects('listen')->with('eloquent.creating: Heritage\Tests\Database\EloquentModelWithObserveAttributeUsingArrayStub', EloquentTestObserverStub::class.'@creating');
        $events->expects('listen')->with('eloquent.saved: Heritage\Tests\Database\EloquentModelWithObserveAttributeUsingArrayStub', EloquentTestObserverStub::class.'@saved');
        $events->shouldReceive('forget');
    ...
    public function testModelObserversCanBeAttachedToModelsThroughAttributesOnParentClasses()
    {
        $events = Mockery::mock(Dispatcher::class);
        $events->expects('dispatch')->times(2);
        $events->expects('listen')->with('eloquent.creating: Heritage\Tests\Database\EloquentModelWithObserveAttributeGrandchildStub', EloquentTestObserverStub::class.'@creating');
        $events->expects('listen')->with('eloquent.saved: Heritage\Tests\Database\EloquentModelWithObserveAttributeGrandchildStub', EloquentTestObserverStub::class.'@saved');
        $events->expects('listen')->with('eloquent.creating: Heritage\Tests\Database\EloquentModelWithObserveAttributeGrandchildStub', EloquentTestAnotherObserverStub::class.'@creating');
        $events->expects('listen')->with('eloquent.saved: Heritage\Tests\Database\EloquentModelWithObserveAttributeGrandchildStub', EloquentTestAnotherObserverStub::class.'@saved');
        $events->expects('listen')->with('eloquent.creating: Heritage\Tests\Database\EloquentModelWithObserveAttributeGrandchildStub', EloquentTestThirdObserverStub::class.'@creating');
        $events->expects('listen')->with('eloquent.saved: Heritage\Tests\Database\EloquentModelWithObserveAttributeGrandchildStub', EloquentTestThirdObserverStub::class.'@saved');
        $events->shouldReceive('forget');
    ...
    public function testModelObserversCanBeAttachedToModelsThroughCallingObserveMethodOnlyOnce()
    {
        ...
        $events->expects('listen')->with('eloquent.creating: Heritage\Tests\Database\EloquentModelStub', EloquentTestAnotherObserverStub::class.'@creating');
        $events->expects('listen')->with('eloquent.saved: Heritage\Tests\Database\EloquentModelStub', EloquentTestAnotherObserverStub::class.'@saved');
    ...
    {
        $events = Mockery::mock(Dispatcher::class);
        $events->expects('listen')->with('eloquent.creating: Heritage\Tests\Database\EloquentModelSaveStub', EloquentTestObserverStub::class.'@creating');
        $events->expects('listen')->with('eloquent.saved: Heritage\Tests\Database\EloquentModelSaveStub', EloquentTestObserverStub::class.'@saved');
        ...
        $events->expects('until')->with('eloquent.saving: Heritage\Tests\Database\EloquentModelSaveStub', $model);
        $events->expects('dispatch')->with('eloquent.saved: Heritage\Tests\Database\EloquentModelSaveStub', $model);
    ...
    {
        $this->expectExceptionObject(new LogicException('Heritage\Tests\Database\EloquentModelStub::incorrectRelationStub must return a relationship instance.'));
    ...
    {
        $this->expectExceptionObject(new JsonEncodingException('Unable to encode attribute [objectAttribute] for model [Heritage\Tests\Database\EloquentModelCastingStub] to JSON: Malformed UTF-8 characters, possibly incorrectly encoded.'));
    ...
    {
        $this->expectExceptionObject(new JsonEncodingException('Unable to encode attribute [jsonAttribute] for model [Heritage\Tests\Database\EloquentModelCastingStub] to JSON: Malformed UTF-8 characters, possibly incorrectly encoded.'));
    ...
    {
        $this->expectExceptionObject(new JsonEncodingException('Unable to encode attribute [jsonAttributeWithUnicode] for model [Heritage\Tests\Database\EloquentModelCastingStub] to JSON: Malformed UTF-8 characters, possibly incorrectly encoded.'));
    ...
    public function testUseCustomBuilderWithUseEloquentBuilderAttribute()
    {
        ...
        $query = $this->createStub(\Heritage\Database\Query\Builder::class);
...
#[\Heritage\Database\Eloquent\Attributes\UseEloquentBuilder(CustomBuilder::class)]
...
#[\Heritage\Database\Eloquent\Attributes\UseEloquentBuilder(ChildCustomBuilder::class)]
