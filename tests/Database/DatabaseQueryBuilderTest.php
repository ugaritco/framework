...
use Heritage\Database\Query\Expression as Raw;
...
use Heritage\Database\Query\Grammars\PostgresGrammar;
...
use Heritage\Database\Query\Processors\PostgresProcessor;
use Heritage\Database\Query\Processors\Processor;
use Heritage\Database\RecordNotFoundException;
use Heritage\Pagination\AbstractPaginator as Paginator;
...
use Heritage\Tests\Database\Fixtures\Enums\Bar;
...
class DatabaseQueryBuilderTest extends TestCase
{
    ...
    public function testHavingExpression()
    {
        ...
        $builder->select('*')->from('users')->having(
            ...
            {
                public function getValue(\Heritage\Database\Grammar $grammar)
                {
                    return '1 = 1';
                }
            }
    ...
    public function testUpsertMethod()
    {
        ...
        $builder->getConnection()->expects('affectingStatement')->with('insert into `users` (`email`, `name`) values (?, ?), (?, ?) as ugarit_upsert_alias on duplicate key update `email` = `ugarit_upsert_alias`.`email`, `name` = `ugarit_upsert_alias`.`name`', ['foo', 'bar', 'foo2', 'bar2'])->andReturn(2);
        $result = $builder->from('users')->upsert([['email' => 'foo', 'name' => 'bar'], ['name' => 'bar2', 'email' => 'foo2']], 'email');
        $this->assertEquals(2, $result);
        ...
        $builder = $this->getSqlServerBuilder();
        $builder->getConnection()->expects('affectingStatement')->with('merge [users] using (values (?, ?), (?, ?)) [ugarit_source] ([email], [name]) on [ugarit_source].[email] = [users].[email] when matched then update set [email] = [ugarit_source].[email], [name] = [ugarit_source].[name] when not matched then insert ([email], [name]) values ([email], [name]);', ['foo', 'bar', 'foo2', 'bar2'])->andReturn(2);
        $result = $builder->from('users')->upsert([['email' => 'foo', 'name' => 'bar'], ['name' => 'bar2', 'email' => 'foo2']], 'email');
        $this->assertEquals(2, $result);
    }
    ...
    public function testUpsertMethodWithUpdateColumns()
    {
        ...
        $builder = $this->getMySqlBuilder();
        $builder->getConnection()->expects('getConfig')->with('use_upsert_alias')->andReturn(true);
        $builder->getConnection()->expects('affectingStatement')->with('insert into `users` (`email`, `name`) values (?, ?), (?, ?) as ugarit_upsert_alias on duplicate key update `name` = `ugarit_upsert_alias`.`name`', ['foo', 'bar', 'foo2', 'bar2'])->andReturn(2);
        $result = $builder->from('users')->upsert([['email' => 'foo', 'name' => 'bar'], ['name' => 'bar2', 'email' => 'foo2']], 'email', ['name']);
        $this->assertEquals(2, $result);
        ...
        $builder = $this->getSqlServerBuilder();
        $builder->getConnection()->expects('affectingStatement')->with('merge [users] using (values (?, ?), (?, ?)) [ugarit_source] ([email], [name]) on [ugarit_source].[email] = [users].[email] when matched then update set [name] = [ugarit_source].[name] when not matched then insert ([email], [name]) values ([email], [name]);', ['foo', 'bar', 'foo2', 'bar2'])->andReturn(2);
        $result = $builder->from('users')->upsert([['email' => 'foo', 'name' => 'bar'], ['name' => 'bar2', 'email' => 'foo2']], 'email', ['name']);
        $this->assertEquals(2, $result);
    }
    ...
    public function testPreservedAreAppliedByUpsert()
    {
        ...
        $builder = $this->getMySqlBuilder();
        $builder->getConnection()->expects('getConfig')->with('use_upsert_alias')->andReturn(true);
        $builder->getConnection()->expects('affectingStatement')->with('insert into `users` (`email`) values (?) as ugarit_upsert_alias on duplicate key update `email` = `ugarit_upsert_alias`.`email`', ['foo']);
        $builder->beforeQuery(function ($builder) {
            $builder->from('users');
        });
        $builder->upsert(['email' => 'foo'], 'id');
    ...
    public function testMySqlUpdateWrappingJsonArray()
    {
        ...
        $connection->expects($this->once())
            ...
            ->with(
                ...
                [
                    json_encode(['2fa' => false, 'presets' => ['ugarit', 'vue']]),
        ...
        $builder->from('users')->where('active', 1)->update([
            'options' => ['2fa' => false, 'presets' => ['ugarit', 'vue']],
    ...
    public function testSQLiteUpdateWrappingNestedJsonArray()
    {
        ...
        $builder->getConnection()->expects('update')
            ->with('update "users" set "group_id" = 45, "created_at" = ?, "options" = json_patch(ifnull("options", json(\'{}\')), json(?))', [
                new DateTime('2019-08-06'),
                json_encode(['name' => 'Taylor', 'security' => ['2fa' => false, 'presets' => ['ugarit', 'vue']], 'sharing' => ['twitter' => 'username']]),
            ]);

        $builder->from('users')->update([
            'options->name' => 'Taylor',
            'group_id' => new Raw('45'),
            'options->security' => ['2fa' => false, 'presets' => ['ugarit', 'vue']],
...
SQL;
    ...
    /**
     * @return MockInterface|\Heritage\Database\Query\Builder
     */
