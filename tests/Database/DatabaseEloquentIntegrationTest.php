...
use Heritage\Database\Eloquent\Model as Eloquent;
use Heritage\Database\Eloquent\ModelNotFoundException;
use Heritage\Database\Eloquent\Relations\MorphPivot;
...
use Heritage\Database\QueryException;
...
use Heritage\Tests\Database\Fixtures\Models\Integration\Post;
...
class DatabaseEloquentIntegrationTest extends TestCase
{
    ...
    public function testBasicModelRetrieval()
    {
        ...
        $this->assertTrue(EloquentTestUser::where('email', 'mohamed@ugarit.com')->doesntExist());
    ...
    {
        $this->expectExceptionObject(new ModelNotFoundException('No query results for model [Heritage\Tests\Database\EloquentTestUser] 1'));
    ...
    {
        $this->expectExceptionObject(new ModelNotFoundException('No query results for model [Heritage\Tests\Database\EloquentTestUser] 2, 3'));
        $this->expectExceptionObject(
    ...
    {
        $user = EloquentTestUser::create(['email' => 'taylor@ugarit.com']);
        $this->connection()->transaction(function () use ($user) {
            try {
                $this->connection()->transaction(function () use ($user) {
                    $user->email = 'otwell@ugarit.com';
                    $user->save();
                    throw new Exception;
                });
            } catch (Exception) {
            ...
            $this->assertSame('taylor@ugarit.com', $user->email);
        });
    }
    ...
    public function testNestedTransactionsUsingSaveOrFailWillSucceed()
    {
        ...
        $this->connection()->transaction(function () use ($user) {
            ...
            $user = EloquentTestUser::first();
            $this->assertSame('otwell@ugarit.com', $user->email);
    ...
    public function testTimestampsUsingOldSqlServerDateFormatFallbackToDefaultParsing(): void
    {
        ...
        // No longer throwing exception since Ugarit 7,
    ...
    public function testCanFillAndInsert()
    {
        ...
        $this->assertTrue(EloquentTestUser::fillAndInsert([
            ['email' => 'taylor@ugarit.com', 'birthday' => null],
            ['email' => 'nuno@ugarit.com', 'birthday' => new Carbon('1980-01-01')],
            ['email' => 'tim@ugarit.com', 'birthday' => '1987-11-01', 'created_at' => '2025-01-02T02:00:55', 'updated_at' => Carbon::parse('2025-02-19T11:41:13')],
        ]));
        ...
        $tim = $users->firstWhere('email', 'tim@ugarit.com');
