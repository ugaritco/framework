...
use Heritage\Tests\Database\Fixtures\Models\Money\Price;
use Heritage\Tests\Http\Fixtures\TestEnum;
use Heritage\Tests\Http\Fixtures\TestEnumBacked;
use Heritage\Tests\Http\Fixtures\TestIntegerEnumBacked;
...
class HttpRequestTest extends TestCase
{
    ...
    {
        $request = Request::create('/', 'GET', [], [], [], [
            'HTTP_USER_AGENT' => 'Ugarit',
        ]);

        $this->assertSame('Ugarit', $request->userAgent());
    }
    ...
    public function testStringMethod()
    {
        ...
        $this->assertInstanceOf(\Heritage\Support\Stringable::class, $request->string('int'));
        $this->assertInstanceOf(\Heritage\Support\Stringable::class, $request->string('unknown_key'));
    ...
    public function testMergeMethodTreatsAsterisksAsLiteralKeys()
    {
        ...
        $request = Request::create('/', 'GET', ['profile' => ['name' => 'Taylor', 'email' => 'taylor@ugarit.com']]);
        $request->merge(['profile.*' => 'masked']);
        $this->assertSame('Taylor', $request->input('profile.name'));
        $this->assertSame('taylor@ugarit.com', $request->input('profile.email'));
    ...
    public function testMergeIfMissingMethod()
    {
        ...
        $request = Request::create('/', 'GET', ['user' => ['first_name' => 'Taylor', 'email' => 'taylor@ugarit.com']]);
    ...
    public function testAllInputPrefersInputOverFilesForCollidingKeys()
    {
        ...
        $request = Request::create('/', 'POST', ['email' => 'taylor@ugarit.com'], [], ['email' => $file]);

        $this->assertSame(['email' => 'taylor@ugarit.com'], $request->all());
        $this->assertSame(['email' => 'taylor@ugarit.com'], $request->only('email'));
    ...
    public function testGetSessionMethodWithUgaritSession()
    {
        $request = Request::create('/');

        $ugaritSession = Mockery::mock(Store::class);
        $request->setUgaritSession($ugaritSession);

        $session = $request->getSession();
        $this->assertInstanceOf(SessionInterface::class, $session);

        $ugaritSession->expects('start')->andReturn(true);
        $session->start();
    }

    public function testGetSessionMethodWithoutUgaritSession()
    {
     ...
     *
     * @link https://github.com/ugarit/framework/pull/7052 Correctly fill the $request->request parameter bag on creation.
     */
     ...
     *
     * @link https://github.com/ugarit/framework/pull/37921 Manually populate POST request body with JSON data only when required.
     */
     ...
     *
     * @link https://github.com/ugarit/framework/issues/10403 Form request object attribute returns empty when have some string.
     */
    ...
    public function testMagicMethodsPreferInputOverFilesForCollidingKeys()
    {
        ...
        $this->assertSame('taylor@ugarit.com', $request->email);
        $this->assertSame('taylor@ugarit.com', $request['email']);
    }
    ...
    public function testItCanHaveObjectsInJsonPayload()
    {
        ...
        $base = SymfonyRequest::create('/', 'POST', server: ['CONTENT_TYPE' => 'application/json'], content: '{"framework":{"name":"Ugarit"}}');
        $request = Request::createFromBase($base);

        $value = $request->get('framework');

        $this->assertSame(['name' => 'Ugarit'], $request->get('framework'));
    }
